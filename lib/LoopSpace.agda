...
module lib.LoopSpace where
  ...
  data WrapPath (A : Type) (a : A) : Type {- +1 -} where
       wrap : {a' : A} (wrapper : Path a a') (center : Path a' a') → WrapPath A a

  unwrap : ∀ {A a} → WrapPath A a → Path a a 
  unwrap (wrap wrapper center) = ! wrapper ∘ center ∘ wrapper
