...
module lib.loopspace.OverTypes where
  ...
  private
    -- it's possible we could want to know the definition of this, but pretty unlikely!
    -- BUT: why does this get unsolved metas if you make it abstact?
    ...
    LoopOverS≃' (S n){A}{a} α B b = 
      ...
      where lemma : apt (S n) (ap^ (S (S n)) B α) b ≃ adj _ 
                                                        (ap (ap^ n (λ f → f b))
  ...
  abstract
    LoopOverS≃ : (n : Positive) {A : Type} {a : A} (α : Loop (S n) A a) → (B : A -> Type) (b : B a) 
               → LoopOver (S n) α B b ≃ LoopOverS n α B b 
    LoopOverS≃ n α B b = LoopOverS≃' n α B b
  ...
  abstract -- possible we could need the definition, but seems unlikely!
    ...
    ap^→ n C D α = LoopSType.ext n (λ f → 
      ...
      apt n (LoopType→ n (ap^ (S n) C α) (ap^ (S n) D α)) f ∎) where
        ...
        ap^-of-o3 n {a = a} {α = α} g1 f g2 = LoopΠ.ext n _ _ (λ x → ! (ap≃ (LoopΠ.β n (λ x → ∘^ n (ap^ n (λ a' → g2 a' (f (g1 a x))) α) (ap^ n (λ a' → g2 a (f (g1 a' x))) α))))
                                                                 ∘ ap^-app n a α (λ t → f (g1 t x)) g2 ∘ ! (ap^-o n (λ f → f x) (λ a x₁ → g2 a (f (g1 a x₁))) α))
