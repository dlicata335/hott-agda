...
module misc.SimplexOp3 where
  ...
  data _⊢_ : Ctx → Ctx → Set where
    ...
    con : ∀ {Ψ1 Ψ2 v} → (⊤ (v opv) :: (rev Ψ1 ++ [ ⊥ (v opv) ])) ⊩ Ψ2 → (Ψ1 , v opv) ⊢ (Ψ2 , v) -- FIXME is this right?
  ...
  compose {Ψ , .(v opv)} {Ψ' , .(v opv)} {Ψ'' , v} (co ρ) (con ρ') =
    con (compose' {Ψ = rev Ψ} {Ψ' = rev Ψ'} {Ψ'' = Ψ''} (transport (λ h → h ⊩ _) (ap (λ h → h ++ [ ⊥ (v opv) ]) (rev-append Ψ [ ⊤ (v opv) ])) (flip ρ)) ρ')
  compose {Ψ , .((v opv) opv)} {Ψ' , .(v opv)} {Ψ'' , v} (con ρ) (con ρ') = 
    transport (\ h → (Ψ , h) ⊢ (Ψ'' , v)) 
              (! (opv-invol {v}))
              (co (transport (λ v' → (⊥ v' :: Ψ ++ [ ⊤ v' ]) ⊩ Ψ'') (opv-invol {v}) (compose' {Ψ = Ψ} {Ψ' = rev Ψ'} {Ψ'' = Ψ''} (transport (λ h → h ⊩ _) (ap (λ h → ⊥ ((v opv) opv) :: append h [ ⊤ ((v opv) opv) ]) (rev-rev Ψ) ∘ ap (λ h → h ++ [ ⊤ ((v opv) opv) ]) (rev-append (rev Ψ) [ ⊥ ((v opv) opv) ])) (flip ρ)) ρ')))
