...
module applications.hopf-scraps.N'S'AreRefl where
  ...
  module Twist2 where
    ...
    fr'body = (S²-elim (λ x → n'body x ≃ s'body x) 
                               fr 
                               {! resp (\ x -> ! s ∘ x ∘ ! n) fr !} -- seems like this has to be a modification of fr
                                                                   -- because (n'body n) and (n'body s) are contentless
...
--                                (resp (\ x -> ! s ∘ x ∘ ! n) fr) ≃〈 {!!} 〉 
    ...
    ba'body = (S²-elim (λ x → n'body x ≃ s'body x) 
                              ba                   -- has to be homotopic to fr for the next level
                              (! (resp ! ba)) 
                              {!!} {!!} {!!} {!!})
