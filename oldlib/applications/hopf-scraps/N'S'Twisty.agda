...
module applications.hopf-scraps.N'S'Twisty where
  ...
  module Loops where
    ...
    collapse-counterclockwise-a-fr = !-inv-l s ∘ resp (λ x → ! x ∘ s) fr

    collapse-counterclockwise-a-ba : (! n ∘ s) ≃ Refl
    collapse-counterclockwise-a-ba = !-inv-l s ∘ resp (λ x → ! x ∘ s) ba
    ...
    collapse-clockwise-b-fr : (n ∘ ! s) ≃ Refl
    collapse-clockwise-b-fr = !-inv-r s ∘ resp (λ x → x ∘ ! s) fr

    collapse-clockwise-b-ba : (n ∘ ! s) ≃ Refl
    collapse-clockwise-b-ba = !-inv-r s ∘ resp (λ x → x ∘ ! s) ba
  ...
  module Twist2 where
    ...
    roty : S² -> S²
    roty = S²-rec b a (! n) (! s) (resp ! ba) (resp ! fr)
    ...
    n'body = 
           (S²-elim (λ x → Id x (rotz x)) 
                     ...
                     (subst (λ p → p ≃ rotz p) n n ≃〈 subst-Id (λ p → p) rotz n n 〉
                      ...
                      ! s ∘ n ∘ ! n ≃〈 ∘-assoc (! s) n (! n) 〉 
                      (! s ∘ n) ∘ ! n ≃〈 resp (λ x → x ∘ ! n) collapse-clockwise-a-fr 〉 -- content
                      Refl ∘ ! n ≃〈 ∘-unit-l (! n) 〉 
                      ! n ∎)
                     (subst (λ p → p ≃ rotz p) s n ≃〈 subst-Id (λ p → p) rotz s n 〉
                      resp rotz s ∘ n ∘ ! (resp (λ p → p) s) ≃〈 resp (λ x → x ∘ n ∘ ! (resp (λ p → p) s)) (Rec.βs _ _ _ _ _ _) 〉
                      ! n ∘ n ∘ ! (resp (λ p → p) s) ≃〈 resp (λ x → ! n ∘ n ∘ ! x) (resp-id s) 〉
                      ! n ∘ n ∘ ! s ≃〈 resp (λ x → ! n ∘ x) collapse-clockwise-b-fr 〉 -- could also be ba: what are the constraints?
                                                                                    -- placed by the next two goals and fr' and ba'?
                      ! n ∘ Refl ≃〈 ∘-unit-r (! n) 〉 
                      (! n ∎)) 
                     {!!} -- FIXME: somewhat worried that these don't work
                     {!!})
    ...
    -- choices for s':
    -- equal to n' [won't work]
    -- one of s/!n or n/!s that has no computational content in the higher identities 
    -- twisty thing for s
    ...
    s'body = 
            (S²-elim (λ x → Id x (rotz x)) 
                     ...
                     (subst (λ p → p ≃ rotz p) n s ≃〈 subst-Id (λ p → p) rotz n s 〉
                        ...
                        ! s ∘ s ∘ ! n ≃〈 resp (\ x -> ! s ∘ x) (collapse-counterclockwise-b-fr) 〉 
                        ! s ∘ Refl ≃〈 ∘-unit-r (! s) 〉 
                        ! s ∎)
                     (subst (λ p → p ≃ rotz p) s s ≃〈 subst-Id (λ p → p) rotz s s 〉
                        ...
                        ! n ∘ s ∘ ! s ≃〈 ∘-assoc (! n) s (! s) 〉 
                        (! n ∘ s) ∘ ! s ≃〈 resp (\ x -> x ∘ ! s) collapse-counterclockwise-a-fr 〉 
                        Refl ∘ ! s ≃〈 ∘-unit-l (! s) 〉 
                        ! s ∎)
                     {!!} 
                     {!!})
...
--                              respd s'body n ∘ resp (subst (λ x → a' x ≃ b' x) n) fr ∘ ! (respd n'body n) ≃〈 def subst ≃ 〉 
--                              ... (resp (\ x -> resp b' n ∘ x ∘ ! (resp a' n)) fr) ... ≃〈 β + unit 〉 
--                              ... (resp (\ x -> ! s ∘ x ∘ ! n) fr) ... ≃〈 {!!} 〉 
                                {!!} ∎) -- plausible by groupoid laws
