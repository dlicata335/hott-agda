...
-- understanding the difference between the ooTopos proof's Codes fibration and the
-- definition of Codes that is closer to the goal you have to prove, where you 
-- pullback the hfibers, rather than defining the three maps.
...
module homotopy.blakersmassey.DifferentCodes (Z X Y : Type) 
                                             (i' j' : _)
                                             (f : Z → X) (g : Z → Y)
  ...
  module Codes' where
    ...
    module Step1 where
      ...
      step2 (z1 , y , p) = ntype (Codes-contr z1 (inr y) p)
  ...
  module Step1Comparison where
    ...
    Step2' = (z1 : Z) (y : Y) (p : inm z1 ≃ inr y) → Contractible (Trunc i+j (HFiber glue-map-r (z1 , y , p)))
    Step2 = (z1 : Z) (y : Y) (p : inl (f z1) ≃ inr y) -> Contractible (Trunc i+j (HFiber glue-map (f z1 , y , p)))
    ...
    step1 = extend i f cf 
              (λ x' →
                 ((y' : _) (p' : _) → Contractible (Trunc i+j (HFiber glue-map (x' , y' , p')))) ,
                  raise-HProp (Πlevel (λ _ → Πlevel (λ _ → Contractible-is-HProp _))))
              (coe eq step2-from-Codes')
          
  theorem : ConnectedMap i+j glue-map
  theorem (x , y , p) = ntype (Step1Comparison.step1 x y p)
