...
module homotopy.blakersmassey.FatFibered (X Y : Type) (P : X → Y → Type)
                                          (i' j' : TLevel)
...
-}
  ...
  module OverZ {x0 : X} {y0 : Y} (p0 : P x0 y0) where
    ...
    gluel-m-eqv : ∀ {x y} (pxy : P x y) {α : Path (inm p0) (inm pxy)} 
               → Equiv (Trunc i+j (HFiber (gluem'-l p0 pxy) α)) (Trunc i+j (HFiber (gluem'-r p0 pxy) α))
    gluel-m-eqv pxy = truncated-HFiber-equiv (gluem'-l p0 pxy) (gluem'-r p0 pxy) {!!} {!!} {!!} {!!} 
...
-- λ pxy0 → ! (gluel pxy) ∘ ! (gluer pxy0 ∘ ! (gluel pxy0)) ∘ gluer p0
    ...
    CodesFor = Pushout-elim _ (λ x α → Trunc i+j (HFiber (gluel' p0) α)) 
                              (λ x y p α → Trunc i+j {! (HFiber (gluem'-r p0 p) α) !})
