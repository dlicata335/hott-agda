...
module homotopy.blakersmassey.ooTopos0 (X Y : Type) (P : X → Y → Type)
  ...
  module OverZMaps {x0 : X} {y0 : Y} (p0 : P x0 y0) where
  ...
  module Codes {x0 : X} {y0 : Y} (p0 : P x0 y0) where
    ...
    glue-l-ml : ∀ {x y} (pxy : P x y) {αm : Path (inm p0) (inm pxy)} {αl : Path (inm p0) (inl x)} (s : ((gluel pxy) ∘ αm) == αl)
              → Equiv (Trunc i+j (HFiber (gluel0) αl)) (Trunc i+j (HFiber (glueml pxy) αm))
    glue-l-ml pxy s = apTrunc' (HFiber-at-equiv (post∘-equiv (gluel pxy)) gluel0) ∘equiv apTrunc' (HFiber-result-equiv (! s))
    ...
    module _ {x y} (pxy : P x y) {α : Path (inm p0) (inm pxy)} where
      ...
      abstract
        m-to-ml-connected = (ConnectedMap.precompose-equiv-connected switchr-equiv
      ...
      glue-m-ml-total : Equiv (Trunc i+j (HFiber gluem-total ((_ , p0) , (_ , pxy) , α)))
                              (Trunc i+j (HFiber glueml-total ((_ , p0) , (_ , pxy) , α)))
      glue-m-ml-total = ConnectedMap.fiber-top-equiv (m-to-ml) gluem-total glueml-total (\ q → m-to-ml-triangle (fst q) (snd q)) m-to-ml-connected
    ...
    glue-r-mr : ∀ {x y} (pxy : P x y) {αm : Path (inm p0) (inm pxy)} {αr : Path (inm p0) (inr y)} (s : ((gluer pxy) ∘ αm) == αr)
              → Equiv (Trunc i+j (HFiber (gluer0) αr)) (Trunc i+j (HFiber (gluemr pxy) αm))
    glue-r-mr pxy s = apTrunc' (HFiber-at-equiv (post∘-equiv (gluer pxy)) (gluer0)) ∘equiv apTrunc' (HFiber-result-equiv (! s))

    module _ {x y} (pxy : P x y) {α : Path (inm p0) (inm pxy)}  where

      glue-mr-mr-total : Equiv (HFiber (gluemr pxy) α) 
                       (HFiber gluemr-total ((_ , p0) , (_ , pxy) , α))
      glue-mr-mr-total = (HFiber-fiberwise-to-total-eqv _) ∘equiv (HFiber-fiberwise-to-total-eqv _)

      abstract
        m-to-mr-connected = (ConnectedMap.precompose-equiv-connected switchl-equiv
            (ConnectedMap.postcompose-equiv-connected (!equiv reassoc-r-eqv) 
              (ConnectedMap.fiberwise-to-total-connected i+j (λ _ → Wedge.wedge-to-prod)
                (λ z → Wedge.WedgeToProd.connected (_ , snd z) (_ , snd z) (cf _) (cg _)))))

      glue-m-mr-total : Equiv (Trunc i+j (HFiber gluem-total ((_ , p0) , (_ , pxy) , α)))
                              (Trunc i+j (HFiber gluemr-total ((_ , p0) , (_ , pxy) , α)))
      glue-m-mr-total = ConnectedMap.fiber-top-equiv (m-to-mr) _ _ (\ q → m-to-mr-triangle (fst q) (snd q)) m-to-mr-connected
         

      glue-m-mr : Equiv (Trunc i+j (HFiber (gluem) ((_ , pxy) , α))) (Trunc i+j (HFiber (gluemr pxy) α))
      glue-m-mr = apTrunc' (!equiv glue-mr-mr-total) ∘equiv glue-m-mr-total ∘equiv apTrunc' (glue-m-m-total pxy)

    glue-m-r : ∀ {x y} (pxy : P x y) {αm : Path (inm p0) (inm pxy)} {αr : Path (inm p0) (inr y)} (d : PathOver (Path (inm p0)) (gluer pxy) αm αr)
             → Equiv (Trunc i+j (HFiber gluem ((_ , pxy) , αm))) (Trunc i+j (HFiber (gluer0) αr))
    glue-m-r pxy d = (!equiv (glue-r-mr pxy (square-to-disc (PathOverPathFrom.out-PathOver-= d))) ∘equiv glue-m-mr pxy)
