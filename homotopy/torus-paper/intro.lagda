...
|Path a b| for the equality type).  A higher inductive types for the
...
can be represented by a higher inductive type (see
\citep{uf13hott-book}) with the following constructors:
\begin{code}
a : T
p : Path a a
q : Path a a
