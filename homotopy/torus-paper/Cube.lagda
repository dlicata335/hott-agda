...
The reason for the order of the faces in |Cube left right back top bot
front| is that we think of a cube as an equality between the left and
right squares, along the ``tube'' given by the |back| and |top| and
|bot| and |front|.  As usual, we could avoid introducing a new inductive
family by instead defining a cube using some square inductions, to say
that when the back, top, bottom, and front are the identity squares, a
cube is a three-dimensional path between the left and the right.
...
left side to the top, and rearranging the other faces as necessary:
\begin{code}
cube-symmetry-left-to-top : Cube left right back top bot front
...
in-PathOver-Square : 
  Cube  f1 f2 
        (out-PathOver-= (apdo p0- δ))
        (out-PathOver-= (apdo p-0 δ)) 
        (out-PathOver-= (apdo p-1 δ))
        (out-PathOver-= (apdo p1- δ)))
