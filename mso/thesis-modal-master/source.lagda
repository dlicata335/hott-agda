...
\par This thesis is progress towards a formal verification of the algorithm given in Courcelle's Theorem. Verification via proof assistants is a method of ensuring that either a mathematical proof or a program is correct that removed much of the possibility for human error. Verification is currently used in a few main situations. The first is  when there is a mathematical proof that is either too long for a human to reliably complete or a completed proof that is not well accepted by the community, for example the Jordan Curve theorem, or the Four Color theorem, which have both been verified by proof assistants [cite{jordan, color}]. Another area of use is for programs run in high-stakes situations, such as medical software, software used by NASA, or that used to create Intel hardware [cite{nasa, intel}]. Finally, proof assistants are also often used simply to record mathematical proofs in a standard fashion. The motivation behind this project is straddled between these latter two goals. Courcelle's Theorem is a large, important mathematical accomplishment, and we hope to add it to the list of verified proofs. However, although the algorithm in Courcelle's Theorem does not have any high-stake implementations that the authors are aware of, the theorem seems to have potential of becoming quite useful in the field of relational databases. If a relational database is created someday that relies upon Courcelle's Theorem to run its queries, a formal verification of the theorem may be useful in situations where queries are being run on sensitive data where far-reaching conclusions may be drawn from the queries on that data (medical data, census data, etc).
...
\par The tool that we use to verify Courcelle's Theorem is called Agda\cite{agda}. Agda is a dependently typed functional programming language based on Haskell. It was created by Ulf Norell in his PhD thesis at Gotenburg University\cite{agda}. Agda uses the rules of Zhaohui Luo's Unified Theory of Dependent Types [cite{luo}] as the underlying theory to prove its programs are correct.\cite{agda}. In order for this logic to remain constistent, Agda also enforced termination checking, i.e. all possible patterns in a program must be matched, and all recursion must be performed on smaller subproblems. Agda's method of termination checking is based on the Foetus termination checker [cite{agda, foetus}].
\par Agda has the some of the usual trappings of regular programming languages, for example, data types, pattern matching, let expressions, and modules. Its syntax is similar to that of Haskell. One quite beautiful feature of Agda, which distinguishes it from other theorem provers, is its use of metavariables in the code. A user can define a function and leave the proof as a metavariable, often referred to as the goal. Agda will display the expected type of the metavariable, an the user can incrementally refine it, by putting pieces of code in bit by bit, and having the expected types of the missing pieces updated by Agda.
\par The introduction will give the formal statement of Courcelle's Theorem and the mathematical definitions required to understand the theorem. We will cover related work and some applications of Courcelle's theorem. Then we will go through the work of this project that has been completed: the formalized definitions in Agda with their explanations a mathematical proof of the overall algorithm, then the work in progress: an explanation and proof of of the entire algorithm in Agda, and finally the work that is left to be done in this project.
...
\par The kinds of questions we can ask are those expressible in Monadic Second Order Logic, or \textbf{MSO}. MSO is an extension of first order logic which, along with all of the capabilities of first order logic, introduces set variables, membership in these sets, and quantification over sets. MSO can also be seen as a restirction of second order logic where quantification is only allowed over unary relations, i.e. sets.  However, no matter which way we look at it,  we cannot quantify over relations that are not sets, i.e. that are not unary. For example, if we modeled edges in a graph $G$ by a binary relation $E(x,y)$ where $x$ and $y$ are vertices, we could not ask the question: $\exists E(x,y)$  such that  $\forall z, z \neq y$ and $x \neq z,$ such that $\neg E(y, z)$ and $\neg E(x, z)$? In other words, is there an isolated edge in $G$? We cannot ask this question because it quantifies over all edges, which here we depict as a binary relation.
\par Even if we do not include quantification over edges, MSO manages to express quite a few NP hard problems: $k$-colorability for fixed $k$, vertex cover, and dominating set, to name a few. Below are the MSO representations of Vertex Cover and Dominating Set. $R$ is a set of verticies, in each formula, and $Edge(x,y)$ denotes the fact that there is an edge between verticies $x$ and $y$, i.e. the tuple $(x,y) \in$ the relation $Edge$.
