...
\par This thesis is a formal verification of the algorithm given in Courcelle's Theorem. The verification is done in Agda, a dependently typed programming language based on Haskell, that is often used a proof-checker\cite{agda}. This work is also an earnest attempt to bring together the computer science disciplines of of programming languages and theory.
\par The introduction will give the formal statement of the theorem and the mathematical definitions required to understand the theorem. We will cover related work and some applications of Courcelle's theorem, then head into the formalized definitions in Agda with their explanations, a mathematical proof of the overall algorithm, then the work in progress: an explanation and proof of of the entire algorithm in Agda, and finally the work that is left to be done in this project.
