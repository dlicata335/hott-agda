...
|StructureS| has four constructors: empty (|[]|), an extension with an element of the underlying set (|,is|, extention with |IndividS A τ|),
an extension with a $nil$ (| ,none |, which mandates that the |StructureS| be open),
and an extension with a relation tuple (|,rs|, with an extension with |IndividsS A τs|).
...
\par Consider the cases in the code where the last symbol in the |Signature| is a constant symbol, but one structure interprets that symbol as $nil$ and the other does not. (f = |Void| and |preserves (A1 , s1 ,none) (A2 , s2 ,is x) f = Void| in the code.) |Void| means false in Agda, so these cases in the code say that |f| cannot exist between two such structures. This corresponds to the part of the mathematical definition which stipulates $\forall$ constants $c \in \Sigma, c^{\mathds{A}}=nil$ if and only if $c^{\mathds{B}}$=$nil$.
...
\par The Agda formulation of $MC$ games is an operation on structures and formulas we call  |⊩c|, or "closed truth". Note that instead of |⊩c| being a game, we simply say that for a |Structure| |(A , SA)| and a formula |φ|,  |(A , SA) ⊩c φ| holds if the verifier of an $MC$ game $MC((A, SA), \phi)$ has a winning strategy. If the falsifier has a winning strategy,|(A , SA) ⊩c φ| fails. This is how we take the game-theoretic element out of the algorithm; we simply model an object very similar to the definition of game given in the code. Other than this, however, the code, below, follows the definition of $MC$ games pretty exactly.
...
\par Now we move onto existentials and universal formulas over constants. If $\phi = \forall c \psi$, for some constant symbol $c$, then in order for |(A , SA) ⊩c φ|, |(A , (SA ,is a)) ⊩c φ| needs to hold for all $a \in A$ (for all |a : IndividS A τ|). This corresponds to the fact that in order for the verifier to have a winning strategy on $MC(\mathds{A},\forall i \psi)$ there needs to be a winning strategy on $MC(\mathds{A_{a}},\psi)$ for all $a \in A$, where $\mathds{A_{a}}$ is the $(\Sigma, c)$ extension structure of $\mathds{A}$. (Since on universal formulas, the falsifier would go first.)
