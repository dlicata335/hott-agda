...
\par The kinds of questions we can ask are those expressible in Monadic Second Order Logic, or \textbf{MSO}. MSO is an extension of first order logic which, along with all of the capabilities of first order logic, introduces set variables, membership in these sets, and quantification over sets. MSO can also be seen as a restirction of second order logic where quantification is only allowed over unary relations, i.e. sets.  However, no matter which way we look at it,  we cannot quantify over relations that are not sets, i.e. are not unary. For example, if we modeled edges in a graph by a binary relation $E(x,y)$ where $x$ and $y$ are vertices, we could not ask the question: $\forall E(x,y)$  is there an element $z, z \neq y$ and $x \neq z,$ such that $E(y, z)$ or $E(x, z)$?  This is because we would be quantifying over all edges, which here we depict as a binary relation.
\par Even if we do not include quantification over edges, MSO manages to express quite a few NP hard problems: k-colorability for fixed k, vertex cover, dominating set, to name a few. Below are the MSO representations of Vertex Set and Dominating Set. $R$ is a set of verticies, in each formula, and $Edge(x,y)$ denotes the fact that there is an edge between verticies $x$ and $y$, i.e. the tuple $(x,y) \in$ the relation Edge.
\[\text{k-Vertex Cover}(G)=\exists R \forall x \forall y (\neg Edge(x , y) \vee x \in R \vee y ∈ R)
\]
\[
\text{k-Dominating Set}(G)= \exists R \forall x(x\in R \vee (\exists y(y\in R \wedge Edge(x, y))))
\]
\par Note that, of course, we can put the edge relation, and other p-ary relations for $r\geq 1$ in the formula, we simply cannot quantify over them. However, there is a way to get around not being able to quantify over edges, which we implement in our code.  We model graphs as simply as a set, where an element of the set could either be a vertex or an edge, instead expressing an edge as  a binary relation. This way, edges are simply set elements, which we can quantify over in MSO logic, instead of a binary relation, which we cannot quantify over. To ensure that the two different types of set elements don't get confused, we give a type to each of them and every quantifier specifies which kind of element it quantifies over.
There are also extensions of MSO, such as ExtendedMSO and LinMSO (Seese, Kniess), that allow for linear optimizations of problems expressible in MSO, for example minimum vertex cover and dominating set. These have also been proven to be decided on graphs of bounded treewidth in fixed-parameter tractable linear time[cite Seese and Kneiss].
...
\]
for some function $f$.  Unfortunately, in the case of Courcelle's Theorem, this function $f$ is very large.  In fact, unless P=NP, the function cannot be upper bounded by an iterated exponential of height bounded by $\phi$ and $t$.
...
\par The concept of treewidth was created by Halin [cite some recent progress] under the name S-function, but it only became prominent through the work of Robertson and Seymour leading up to their famous Graph Minor Theorem. A minor of a graph $G$ is any graph $G'$ that can be produced by a series of the following three operations: deleting an edge, contracting an edge, and deleting an isolated node [lovasz]. Robertson and Seymour set out on what ended up being a twenty-five year (and twenty-three-paper) long journey to prove something called Wagner's Conjecture, which became the Graph Minor Theorem. Wagner's Conjecture is based off of Kuratowski's Theorem in graph theory, which will help set up the intuition for the theorem. Kuratowski's Theorem states that a graph $G$ is embeddable in the plane if and only if it does not contain a subgraph homeomorphic to the complete graph $K_{5}$ or the complete bipartite graph $K_{3,3}$. [kuratowski] Wagner's Conjecture asks for a much more general principle: that if a class of graphs $K$ is minor-closed (i.e. $\forall G \in K, G'$ is a minor of $G,$ then $G' \in K$), then $K$ can be characterized by a finite number of excluded minors. Robertson and Seymour re-discovered treewidth in this context, as a purely mathematical tool to prove Wagner's Conjecture. Graph Minor Theory by Laszlo Lovasz is a well-written, concise introduction to graph minor theory and Robertson and Seymour's results from a purely mathematical perspective. [cite] Graph minors have their own applications to computer science via coloring problems and combinatorial optimization [kawarabashi 25, 31], but computer scientists soon picked up on the notion of treewidth much more widely as a way of circumventing NP hardness.[cite??] Some Recent Progress and Applications in Graph Minor Theory is a more extended resource to learn about the landscape of graph minor theory beyond just Robertson and Seymour's work [cite]. \textcolor{red}{need more stuff about treewidth applications more generally?}
\par Graph Rewriting: An Algebraic and Logical Approach by Courcelle provides a background to expressing graphs as logical structures, which is how Courcelle's Theorem interprets graphs. It explains which graph properties are definable in first order logic (for example, having degree k is first order logic expressible, but not connectedness), monadic second order logic (e.g. connectedness, but not having the same number of edges in label sets A and B) and second order logic (e.g. that a graph is finite)[rewriting 199, 200]. In Easy problems for tree-decomposable graphs by Lagergren and Seese, it is noted that MSO formulas can express the notion of k-colorability or k-vertex set, but because one cannot count in MSO, one cannot express the minimum vertex set or the minimum coloring, which is the most common version of these problems. Lagergarden and Seese extend MSO to something called extended monadic second order logic (EMS) which allows counting, and prove that Courcelle's Theorem extends to proving EMS properties on graphs of bounded treewidth in FPT linear time. So, it is also possible to solve minimum vertex cover and minimum dominating set, and a host of other optimization problems using this extension of Courcelle's Theorem.[cite] This formulation of Courcelle's Theorem is the most widely used, but in this paper we only verify the original version.
\par  Parametrized complexity, the field that gives us the notion of fixed parameter tractable algorithms, was developed by Robert Downey and Michael Fellows in the 1990s[cite fellows book]. The main idea in paramtertized complexity is thinking of complexity in multiple dimensions rather than just one. Downey and Fellows explain their idea as a "deal with the devil" to compensate for the inevitable combinatorial explosion in most interesting algorithms: they limit the explosion to one attribute, the parameter, of the input. [cite downey fellows book]. In our case, the inevitable exponential blowup is checking every element of an underlying set of a structure at each universal or existential quantifier is limited to the treewidth of the graph and the encoding of the MSO sentence.
\par In Downey and Fellows' parametrized complexity book, they demonstrate one of many ways to prove Courcelle's Theorem, using the Myhill-Neurode theorem and the method of test sets.[cite] However, this method has never been implemented. There are several different ways to prove, and implement, the Theorem. Perhaps the most intuitive, or understandable, way is through a reduction to the classical model checking problem for MSO on labeled trees which is explained in the paper by Lagergren and Seese[cite, grohe, query]. There is a tool called MONA that implements this approach, with an attempt to make the algorithm somewhat practicable[17 in kniess]. However, this algorithm is still almost unusable because the exponential constant made from the treewidth and the encoding of the MSO sentence makes the algorithm take up incredible amounts of time and space.[kniess]
\par The problem with the automata approach is that in practice, many of the states of the tree automaton generated are never used (cite). There are a few different ways to circumvent this issue; they all involve some version of creating what you need as you go. Courcelle and Durand use functions instead of tables to model automata in an effort to make the algorithm faster and dub the method using fly-automata [automata for the verification]. The name is due to the fact that the  state-transition functions are created "on-the-fly". Alternatively, this can be done by scrapping the idea of automata all together. Ganzow and Kaiser bypass the use of automata at all and directly manipulates the input formulas [new algo].
\par This is also the approach taken in Courcelle's Theorem-- A Game-Theoretic Approach by Kneis, Langer, and Rossmanish, which is the algorithm we use to verify Courcelle’s Theorem in our work. Kneis et. al. come up with a new strategy to tackle Courcelle’s Theorem, borrowing tools from Game Theory, namely, model checking games. The algorithm they outline to decide the formula on the structure, at its heart, a  relatively simple dynamic programming algorithm. As mentioned above, the problem with the automata theoretic approach is that many of the states created are never actually used to decide the formula on the input graph, so prohibitively large amounts of time and space are spent creating things that will never actually be used. Kneis et. al. suggest this is because this proof and implementation method do not pay attention to the attributes of the specific graph and MSO formula given, the automata theoretic implementation does a very similar thing no matter what the input is. To fix this problem, Kneis et. al. take the details of the input into account, and in light of the input, systematically trim down the information stored to be only what is necessary. As alluded to earlier, this implementation actually has some experimental evidence showing that the algorithm runs in a usable amount of time (from a few seconds to under 24 hours) for graphs up to treewidth 12 for minimum vertex cover, dominating set, and 3-colorability.[cite] An explanation of the algorithm is in the last section of this paper, however, we give a quick intuitive idea. At each node $i$ of the tree decomposition, there is a bad $X_{i}$ of vertices of the original graph. Additionally, we can associate with each node a set $A_{i}$, which is the union of all bags on the tree starting at the leaves and up until, and including, the bag $X_{i}$. The paper shows that if an MSO sentence $\phi$ is shown to hold over a restriction of the graph $G$ to the set  $A_{i}$, called $G[A_{i}]$--a notion that we will formalize later but essentially means that $G[A_{i}]$ is the graph $G$ with all the elements that are not in $A_{i}$ thrown out of it--then $\phi$ is true over the whole graph. And similarly if the $\phi$ can be shown to not hold over $G[A_{i}]$.  (The way in which the formula is “shown to hold” is via a model checking game, which again, we will explain in the pages to come.) So, the algorithm traverses the tree decomposition bottom up, evaluating whether or not $\phi$ holds for each $G[A_{i}]$ via a game: if the formula holds, the algorithm exits and returns that the formula holds, similarly if the formula is shown to be false on the graph; and if the data is insufficient, then there is a special combining algorithm that combines the game on $G[A_{i}]$ with the game on $G[X_{h}]$, where $h$ is the next node above $j$. Note that the combining algorithm takes the recursive call and only the $bag$ at the next node, not the entire set of all the bags below it. Because of this, the algorithm never checks the formula over the entire graph, only over the subsets of the graph in the bags $X_{i}$ of the tree decomposition. Each such calculation is “constant time” because it is not related to the size of the graph, only the tree width, which is one of our parameters. Thus, the algorithm is linear in the size of the input graph—because the number of nodes in the input graph is related in a linear to how many nodes there will be in the tree decomposition[cite?] and this process of playing a game on a restricted structure happens at each node of the tree decomposition. But, since playing the model checking game on a node will be constant time because the size of the node is always less than the parameter of tree width, the algorithm is linear over all. Unfortunately, the algorithm on these restrictions is exponential in the size of the bags, which is where the nasty exponential constant in the size of the parameters partially comes from. However, the algorithm in Kneis et. al. tackles this problem as well, by introducing a notion of isomorphism between games, so that we have to keep track of as few games as possible, which cuts down on this constant. The details of these ideas will be explained in the pages to come.
\par As far as the authors are aware, there are no other attempts to formally verify Courcelle's Theorem. There have been papers written about verifying different processes related to Courcelle's theorem, for example, Martin Streker verfied some systems of graph rewriting, which is central to Courcelle's Theorem. But there is no literature we can find on tackling Courcelle's Theorem specifically.
\par Some application areas of Courcelle's Theorem include graph theoretic algorithms: for example, a paper on constructing graph minor decompositions by Reed et. al. uses Courcelle's Theorem in the proof [cite reed paper]. Another application area is  phylogenetics, where scientists need to answer questions about graphs called agreement forests, which depict how close two different proposed phylogenetic relationship graphs are. The questions they need to ask are expressible in MSO, and in practice, one researcher Stephen Kelk points out that agreement forests often have low treewidth. A good, fast implementation of Courcelle's Theorem, then,  would likely speed up the research of phylogeneticists [cite slideshow]. Another potentially exciting area of application is to databases theory, which deals mostly with first order logic questions asked about databases, which are of course covered by MSO. Courcelle's Theorem immediately extends to relational databases with bounded treewidth[cite database theory book '99]. So, Courcelle's Theorem promises a faster implementation for SQL queries, which, if brought to fruition, could potentially cause a huge increase in querying speed.[cite thingy]
\section{Definitions}
...
\par It has been proven that the verifier has a winning strategy on a model-checking game $MC(\mathds{A}, \phi)$ if and only if $\mathds{A} \vdash \phi$ [cite thing].
\section{Outline of Algorithm}
\par While we will prove our own version of the overall proof of the algorithm with our code, we outline the proof from the paper here, which is never explicitly put together for this method of proving Courcelle's Theorem. However, in an effort not to reproduce too much of their paper, we simply rely upon several lemmas from the paper by Kneiss et. al. We reproduce the statement of the lemmas below, but for the proof, please refer to the Kneiss et. al. paper. Additionally, note that we refer to two algorithms not mentioned yet, "combine" and "combine-forget". These are algorithms laid out in the Kneiss et. al. paper which we will not reproduce, but please think of them as similar to the agda functions combineIntro, combineJoin, and combineForget which will be described below.
...
\begin{code}
naive : ∀ {Σ'} {oc} → (φ : Formula Σ') (A : Structure oc Σ') → (X : fixed1 (fst A)) → Either (Either (A ⊩o φ) (A ⊩o φ false)) (provesR A φ X)
\end{code}
