...
% Setup {{{
  ...
  %include agda.fmt
  \usepackage{amsmath, amsthm, tikz, natbib, textgreek, bussproofs, epigraph, color, float,
...
Dan: Thank you for introducing me to computer science, inspiring me to pursue it, and helping me stay afloat in it. Thank you for going along with my request to work on graph algorithms with you, a programming languages theorist. Thank you for long meetings and frequent emails. Thanks for cutting me a lot of slack in my darkest hours. My path would have been quite different without you, and I'm so grateful that it is the way it is. \\
Mom and Dad: Thanks for always, always being there. \\
Thienthanh: Thank you for merging into one consciousness with me: eating together, working together, meditating together, moping together, inspiring hope together for the last two weeks. I don't know if I could have finished without you by my side day in and day out.\\
Max and Joomy: Thanks for all the venting, commisterating, pep-talking, story-telling, and knowledge-sharing that happened in our office. \\
Professor Krizanc: Thank you very much for guiding me towards the topic of my thesis, for spending many hours helping me over the summer, and for giving me very helpful advice about graduate school and life in general. \\
Professors Krizanc and Manfredi: Thank you so much for agreeing to read my thesis! \\
Leyla: Thank you for feeding me and buying my champagne.
...
\subsection{What Kinds of Questions: Monadic Second Order Logic Questions}
\par The kinds of questions we can ask are those expressible in Monadic Second Order Logic, or \textit{MSO}. MSO is an extension of first order logic which, along with all of the capabilities of first order logic, introduces set variables, membership in these sets, and quantification over sets. MSO can also be seen as a restriction of second order logic where quantification is only allowed over unary relations, i.e. sets.  However, no matter which way we look at it,  we cannot quantify over relations that are not sets, i.e. that are not unary. For example, suppose we model a graph $G$ as a set of verticies $V$ and a binary relation $E(x,y)$, where $x$ and $y$ are vertices, to symbolize edges. Then suppose we wanted to see if there existed a binary relation $U(x,y)$ on $G$ such that
\[ \exists U(x,y)(U(x,y) \leftrightarrow E(x,y)\wedge E(y,x))
\]
In other words, are there bidirectional edges in $G$? We cannot ask this question because it quantifies over a binary relation, $U$. However note that if we instead model $G$ as just one set where set elements can represent vertices or edges, we can recast this formula in MSO as shown below. Here $ends(e,x,y)$ is a ternary relation where $e$ is a set element representing an edge, and $x,y$ are set elements represeting vertices, and $ends(e,x,y)$ represents the fact that $e$ is an edge between $x$ and $y$.
\[ \exists P(e)\forall e (P(e) \leftrightarrow \exists x, y, e'(ends(e,x,y) \leftrightarrow ends(e',y,x)))
\]
So including edges in our set, instead of casting them as a relation, expands the set of formulas that Courcelle's Theorem applies to. This quick fix is taken from Courcelle\cite{graphrewrite}, and we implement it our code. Instead of expressing edges as a binary relation, we model graphs as simply a set, where the set elements can represent veritices or edges of the graph. This way, edges are simply set elements, which we can quantify over in MSO logic, and so we can express more formulas than by representing edges as a relation. Note that this requires having a relation $ends$ that essentially labels edges with the vertices they connect. Additionally, in order to ensure that the two different types of set elements do not get confused in our code, we have set elements carry a bit of data with them, saying what type (node or edge) they are.
\par Even without quantification over non-unary relations or the quick fix mentioned above, MSO manages to express quite a few NP hard problems: $k$-colorability for fixed $k$, vertex cover, and dominating set, to name a few. Below are the MSO representations of Vertex Cover and Dominating Set. $R$ is a set of verticies, in each formula, and $Edge(x,y)$ denotes the fact that there is an edge between verticies $x$ and $y$, i.e. the tuple $(x,y) \in$ the relation $Edge$.
...
\par A tree decomposition of a graph $G=(V, E)$ is a pair $(T, X)$ where $T$ is a tree and $X$ is a family of sets indexed by the vertices of $T$, such that
\begin{enumerate}
\item $\bigcup X_{i} = V$
...
\end{enumerate}
\par The sets of vertices, $X_{i}$, are called bags. The width of a tree decomposition is the size of the largest bag $X_{i}$ minus one. The \textit{treewidth} of a graph is the minimum width over all possible tree decompositions.



\par We can see that a tree decomposition of a graph is an encoding of a graph into a tree: it preserves all the information of the original graph within it. It has all of the vertices of the original graph by the first condition. It keeps track of all of the edges in the graph by the second condition. To see why the third condition is necessary, we need to give an overview of what the algorithm given by Courcelle's Theorem looks like. The idea is that we check the input formula $\phi$ over only the part of the structure present in each bag $X_{i}$ of a tree decomposition, and merge that information in a way which will be explained later. In light of this, to illustrate why condition three is needed, consider the two figues below. We have a graph $G$ that is a triangle, and a "tree decomposition" $T$ which satisfies the first and second, but not the third, condition of tree decompositions.
\begin{figure}[h]
\caption{Our graph G}
\[\begin{tikzpicture} \draw (0, 1) node {$\bullet$} -- (1, 0) node {$\bullet$} node [right] {$z$} -- (-1, 0) node {$\bullet$} node [left] {$y$} -- (0, 1); \node at (0, 1.075) [above] {$x$}; \end{tikzpicture}\]
\end{figure}
\begin{figure}[h]
\caption{Our "tree decomposition" $T$}
\includegraphics[width=2in]{tree_dec.pdf}
\end{figure}
\par Suppose the formula we wanted to check on this graph was
\[\exists x,y,z(Edge(x,y) \wedge Edge(y,z) \wedge Edge(z,x))
\]
i.e. that there is a triangle in $G$. See that there is no bag of $T$ that tells us to check the formula on $\{x,y,z \}$ all at once--so we never come across the information that $\{x,y,z \}$ form a triangle, and the algorithm would return that there is no triangle in $G$. However, if we enforce condition three, then we would have to change the root of $T$ to have bag $\{x,y,z\}$, so we would come across the information, and return the correct answer. So, we see that condition three is required.
\par We include an example of a proper tree decomposition. Consider the graph below, $G$. We can see it is certianly not a tree, as it has cycles. However, we want to encode $G$ as a tree, and preserve all of its information.
...
\begin{thm}{Courcelle's Theorem\cite{kneis, cour, gro}}
...
\par Due to this fact, a large portion of this work will be dedicated to explaining the definitions we have created, and convincing the reader that they correctly represent the mathematical definitions so that our proof may be trusted.
\par The tool that we use to verify Courcelle's Theorem, as we mentioned above, is called Agda\cite{agda}. Agda is a dependently typed functional programming language based on Haskell. It was created by Ulf Norell in his PhD thesis at Gotenburg University\cite{agda}. In order for Agda's underlying logic to remain constistent, Agda also enforced termination checking, i.e. all possible patterns in a program must be matched, and all recursion must be performed on smaller subproblems.
\par Agda has some of the usual trappings of regular programming languages, for example, data types, pattern matching, let expressions, and modules. Its syntax is similar to that of Haskell. One quite beautiful feature of Agda, is its use of metavariables in the code. A user can define a function and leave the proof as a metavariable, often referred to as the goal. Agda will display the expected type of the metavariable, and the user can incrementally refine it, by putting pieces of code in bit by bit. Agda will update the expected types of the remaining missing pieces.
\subsection{Contributions and Future Work}
\par The goal of this project is to complete a formal verification of Courcelle's Theorem in Agda. At this stage, we have completed our translation of the definitions given in Courcelle's Theorem: A Game Theoretic Approach\cite{kneis} into Agda. This involved reformulating several complex mathematical concepts into machine-checkable definitions. Specifically, we define symbols, signatures, structures, expansions of structures, restrictions of structures, tree decompositions, MSO formulas, isomorphisms of structures, model checking games, extended model checking games, equivalence between game positions and subgames, and finally a notion of a reduced game. The bulk of this thesis document is a detailed explanation of the fact that the mathematical definitions we formalize correspond to the way we encode them. This is in an effort to convince the reader that, once we do finish the complete proof, it will be correct since our definitions are correct, for the only way a proof assistant can be incorrect is by the author of the program unintentionally encoding the wrong statement of the theorem. Additionally, we have defined the types of the functions corresponding to the lemmas in the paper that prove Courcelle’s Theorem, however we have yet to implement them. We also have yet to prove the fact that the algorithm is fixed-parameter tractable linear; this is next on our agenda after completing the proof of the algorithm itself. After this work is complete, we hope to extend our verification of Courcelle’s Theorem to some of its applications. One probable direction we will pursue is verifying the algorithm in the paper above by Grohe et al. We are also considering applications to phylogenetics and database theory, upon greater research into the applications of Courcelle's Theorem.
\subsection{Road Map}
\par The rest of the paper will proceed in the following manner:
\par Chapter Two, Helpful Background and History, will expand a little bit on the three disciplines that this theorem draws upon. The first is graph minor theory, which is the origin of tree decompositions. The second is (monadic second order) logic, and some extensions of Courcelle's Theorem which apply to questions formulated in slightly more expressive logics. We also touch on logic's connections to graphs via graph rewriting,  which is how Courcelle's Theorem takes in its input graphs. Finally, we will touch on parametrized complexity, which is where we get the notion of fixed-parameter tractable linear from. After briefly engaging with these topics, we will allude to some common methods of proving Courcelle's Theorem, and note their strengths and weaknesses.
\par Following this, in Chapter Three, we give an outline of the proof of Courcelle's Theorem in Courcelle's Theorem: A Game Theoretic Approach by Kneis et al.\cite{kneis}, which is the proof method we base our verification off of.
\par In Chapter Four, Related Work and Applications, we touch on other verification work in this space and some applications of Courcelle's Theorem in graph theoretic algorithms, phylogenetics, and database theory  before heading into the main part of the paper.
\par Chapter Five, Definitions, contains the bulk of the work of this thesis. Here, we go through the concepts defined in the Kneis et al. paper\cite{kneis}, present our translations of each concept in Agda, and show how the two are equivalent. This is in an effort to convince the reader that, once we do finish the complete proof, it will be correct since our definitions are correct. Specifically, we define symbols, signatures,  structures, and expansions of structures (3.1);  restrictions of structures (3.2); tree decompositions (3.3); MSO formulas (3.4), isomorphismis of structures (3.5); compatibility (3.6); structure union (3.7); games, model checking games, and extended model checking games (3.8); and finally, position equivalence,  game equivalence, and reduced games (3.9).
\par Our last chapter before the conclusion is Chapter Six, Outline of Algorithm and Work in Progress. Here, we give a mathematical proof of Courcelle's Theorem based on the lemmas outlined by Kneis et al. \cite{kneis}. Then we present with our progress on the proof in Agda, i.e. the types of the functions in Agda with which we will prove the theorem and how they fit together.
\chapter[Background]{Helpful Background and History}
\par The concept of treewidth was created by Halin \cite{sfunc} under the name S-function, but it only became prominent through the work of Robertson and Seymour leading up to their famous Graph Minor Theorem.\cite{recentprog} A minor of a graph $G$ is any graph $G'$ that can be produced by a series of the following three operations: deleting an edge, contracting an edge, and deleting an isolated node \cite{graphminor}. Robertson and Seymour set out on what ended up being a twenty-five year- (and twenty-three-paper-) long journey to prove something called Wagner's Conjecture, which became the Graph Minor Theorem. Wagner's Conjecture is based on Kuratowski's Theorem in graph theory, which will help set up the intuition for the theorem. Kuratowski's Theorem states that a graph $G$ is embeddable in the plane if and only if it does not contain a subgraph homeomorphic to the complete graph $K_{5}$ or the complete bipartite graph $K_{3,3}$ \cite{kuratowski}. Wagner's Conjecture asks for a much more general principle: that if a class of graphs $K$ is minor-closed (i.e. $\forall G \in K, G'$ is a minor of $G,$ then $G' \in K$), then $K$ can be characterized by a finite number of excluded minors. Robertson and Seymour re-discovered treewidth in this context, as a purely mathematical tool to prove Wagner's Conjecture. Graph Minor Theory by Laszlo Lovasz is a well-written, concise introduction to graph minor theory and to Robertson and Seymour's results from a purely mathematical perspective\cite{graphminor}. Graph minors have their own applications to computer science via coloring problems and combinatorial optimization\cite{recentprog}, but computer scientists soon picked up on the notion of treewidth much more widely as a way of putting a contraint on graphs in order to find faster algorithms that take advantage of this constraint. Some Recent Progress and Applications in Graph Minor Theory is a more extended resource to learn about the landscape of graph minor theory beyond just Robertson and Seymour's work \cite{recentprog}.
...
\chapter{Definitions}
\par In this chapter, we systematically go through the definitions stated by Kneis et al.\cite{kneis}, present our Agda translations of these definitions, and then explain how the two connect. These definitions feed into one another, both mathematically and in the code, and we do our best to present the definition in an intuitive fashion. Broadly, the first three groups of definitions, Structures (which includes extensions and restrictions), Tree Decompositions, and MSO Formulas, are the input types to the overall algorithm. As we will see below, structures are a generalized class of objects that include graphs, so structures take the place of graphs in our in our input definitions. The next few definitions, Isomorphisms of Structures, Compatibility, and Union of Structures, is how we can meaningfully relate structures to one another. The next section, Games, including Model Checking Games and EMC Games, describes the objects that the Kneis algorithm and our code use to determine if a formula is true on a structure. Finally, the reduced games definitions show how we can limit the information we can store in our games, i.e. in our methods of proving formulas true on structures, in order to save time in the algorithm.
\section{Structures}
\par The version of Courcelle's Theorem that is verified in this paper generalizes beyond graphs as the input object to the algorithm, and instead has something called a $structure$ as input. We show that any MSO sentence can be decided on a structure of bounded treewidth in FPT-linear time. In order to define structures, we have to introduce some other concepts first.
...
\begin{code}
  ...
  data SigThing : Type where
    ...
    r : List Tp → SigThing
\end{code}
\par Now we come to our Agda definition of a signature. A |Signature| in Agda is a list of |SigThings|, i.e. a list of symbols. And, a list is finite by definition, so we satisfy the requirement that the signature is a finite set of symbols. We have two functions on |Signature|s, which allows us to extend them: the function |,i| allows us to add another constant onto our |Signature|, and the function |,r| allows us to add another relation onto our |Signature|. These functions take a |Signature| and either a constant or a relation, and return a |Signature| which is the original |Signature| appended with the constant.
\begin{code}
...
\par After defining |Subset|, we defined |IndividS|, which is simply an |Individ| together with the data that it is in a specified |Subset|. This will become useful later on.
\begin{code}
...
\end{code}
\par We also defined a datatype |OC|, with constructors open and closed. This datatype will be part of the information that the Agda type for structures carry with them. A structure being open corresponds to possibly it having constants interpreted as $nil$, and a structure being closed means that all constants in $\Sigma$ are interpreted in $\mathds{A}$.
...
\par Then, combining |Subset|, |OC|, and |Signature|, we defined something called a |StructureS|, which we build on to finally define a structure.
...
Now would be a useful time to review the notion of expansions of structures given slightly above this definition.
|StructureS| has four constructors: empty (|[]|), an expansion with an element of the underlying set (|,is|, expansion with |IndividS A τ|),
an expansion with a $nil$ (| ,none |, which mandates that the |StructureS| be open),
and an expansion with a relation tuple (|,rs|, with an expansion with |IndividsS A τs|).
Note that the constructors with extentions return a |StructureS| over an extended |Signature|,
adding a constant symbol of type |τ| in the |,is| and |,none| cases (|Σ ,i τ|), and adding a relation symbol |r| in the |,rs| case (|Σ ,r τs|).
...
\par Suppose we have a structure $\mathds{A} = ( A, (R^{\mathds{A}})_{R\in rel(\Sigma)}, (c^{\mathds{A}})_{c\in constants(\Sigma)})$. We can restrict this structure to a smaller subset $A_{1} \subseteq A$, and call this structure $\mathds{A}[A_{1}]$. In this new structure, we insist that we only have the set elements in $A_{1}$, and so we only have the constant and relation interpretations that are contained entirely inside $A_{1}$. Formally, given a structure $\mathds{A}$ and a set $A_{1} \subseteq A$, the restriction of $\mathds{A}$ to $A_{1}$, $\mathds{A}[A_{1}]$, is a structure with universe $A_{1}$,  $(R^{\mathds{A}[A_{1}]})= R^{\mathds{A}} \cap A_{1}^{arity(R)}$, and $c^{\mathds{A}[A_{1}]} = c^{\mathds{A}}$ if $c^{\mathds{A}} \in A_{1},$ and becomes interpreted as $nil$ otherwise.


\par In Agda, we encode this notion by a helper function |restrictionS| on |StructureS|'s, and from that we build the function |restriction| on |Structure|s. |restrictionS| requires a |StructureS A1|, a subset |S1|, a proof that |S1| is decidable (|DecidableSub S1|), and a proof that |S1| $\subseteq A$ (|Sub S1 (A1)|). It returns the interpretations of |A1| restrictied to the set |S1|. Note that in our helper function |restrictionS|, we ask for a proof that the subset we restrict to is decidable; when we fix our notion of subset this will no longer be necessary. The code is relatively straightforward and there is no need to go through it in detail; essentially this code runs through each |IndividS| that interprets a constant in the |Signature| in the |StructureS A1'|, and if it is not in the restriction subset |S1|, it replaces this interpretation with a $nil$. And it only includes relation tuples that are entirely within |S1|. This is because in the last case, we stipulate that |v| (in |(λ v → U (promoteIndividsS sb v))|), which is the tuple of set elements that interprets the relation |rs|, has type |IndividsS|, in |S1|. (Having type |IndividsS| in |S1| means that |v| is tuple of set elements, |Individs|, in |S1|.) This is done by the function |promoteIndividsS|, which we will not go into, but shuttles the type of set elements between sets---i.e. if $a$ is a set element of $A$, and $B \subseteq A$, then certainly $a$ should be a set element of $B$ as well. |promoteIndividsS| ensures this.
\begin{code}
...
\end{code}
\par The code for |restriction| simply attaches the appropriate underlying |Subset|, |S1|, to the restricted interpretations given by |restrictionS|, |StructureS Open S1 Σ|. |restriction| takes essentially the same inputs as |restrictionS|, except that it requires and returns a |Structure| and not a |StructureS|.
\begin{code}
...
\par Recall our definition of tree decompositions above. The formal definition upon which we based our code is more general than the definition given above, because the game-theoretic algorithm works over all structures and not simply over graphs. The definition for a tree decomposition of an arbitrary structure is as follows:
\par A tree decomposition of a structure $\mathds{A}$ over a signature $\Sigma$ is a tuple $(\textbf{T}, X)$, where $\textbf{T}=(T,F)$ is a rooted tree and $X=(X_{i})_{i \in T} $ is a collection of subsets $X_{i} \subset A$, (where A is the underlying subset of $\mathds{A}$) such that
\begin{enumerate}
\item $\bigcup X_{i} = A $
...
\item If $i, j, k$ are nodes of T such that $i$ is on a path between $j$ and $k$ in $\textbf{T}$, then $(X_{j} \cap X_{k}) \subseteq X_{i}. $
\end{enumerate}
\par Furthermore, in the algorithm that Kneis et al.\cite{kneis} outline, they use only a type of tree decompositions called nice tree decompositions. Nice tree decompositions are directed, and every edge is directed away from the root. Nice tree decompositions have exactly four types of nodes: leaves, forget nodes, introduce nodes, and join nodes.
\begin{itemize}
...
\caption{A nice tree decomposition of the graph $G$ from Figure \ref{graph}}
\end{figure}
\par Finally, keep this idea in mind: for every node $i$ of a nice tree decomposition of a $\Sigma$-structure $\mathds{A}$, let $A_{i} = \bigcup_{j \text{ at or below } i} X_{j}$. See that $A_{i} \subseteq A$. Then for every node $i$, we can associate with $i$ a substructure $\mathds{A}[A_{i}]$, i.e. a restriction of $\mathds{A}$ to $A_{i}$.
\par In Agda, we modeled a nice tree decomposition as a datatype with four constructors, |Empty|, |Delete|, |Intro|, and |Join|, one for each type of node. Each node carries two subsets with it; its bag (|X|) and the set of all elements in all bags at and below it (|B|). More precisely, at node $i$, |B|=$ \bigcup_{n\text{ at or below } i} X_{n}$. We will prove that this is the case after we give an explanation of each constructor.
\begin{code}
...
\end{code}
\par The |Empty| constructor corresponds to the mathematical definition of the leaf of a tree decomposition. It has an empty set for its bag and an empty set for the elements below. This corresponds to the definition of a nice tree decomposition given above: the leaves are required to have no nodes below them, necessitating that $B = \bigcup_{\text{j at or below i}}X_{j}=\emptyset$, and and also to have empty bags, necessitating that $X_{i}=\emptyset$.
\par The delete constructor, i.e. the forget node, takes a tree decomposition of a bag $X_{i} \cup x$ and a set $B$ of elements below, and returns a tree decomposition with just the bag $X_{i}$ and the same set $B$ of elements below.
\par The |Intro| constructor, for the introduce node, requires a few additional pieces of information. Firstly, it needs a proof (|xnew|) that the element (|x|) being introduced is indeed new. In the code this is described as |x| being an element of the complement of |B|, the set of objects below the |Intro| node in the tree decomposition. Secondly, it needs a proof showing that for all relations $R$ that have tuples made up exclusively of |x| and and elements of |B|, all of the elements in these tuples are in fact contained in $X \cup {x}$. This corresponds to the next five lines of code after |xnew|. This condition is necessary in order to satisfy the second and  third conditions of the definition of tree decompositions. To illustrate this, consider some introduce node $i$. Suppose the second proof were false, i.e. there exists some tuple $p$ consisting exclusively of elements of $B$ and $x$ such that $p \notin X_{i}$. Recall that the second condition of a tree decomposition requires that every tuple of every relation be in some bag $X_{j}$. We know by the first required proof that $x$ is not in $B$, so any relation containing $x$, including $p$, must be either in $X_{i}$ or in some $X_{j}$ above $X_{i}$. So since $p\notin X_{i}$, $p$ must be in some some $X_{j}$ above $X_{i}$. Suppose one of the elements $b \in B$, $b \in p$ is in some bag $X_{h}$, with $h$ below $i$ in $T$. Suppose $b \notin X_{i}$. Then $b$ must be in $X_{j}$, since $b \in p$. Then, by condition three of tree decompositions, since there is a path between $X_{h}$ and $X_{j}$ through $X_{i}$,  $(X_{h} \cap X_{j}) \subset X_{i}. $  This means that $b \in X_{i}$. But this is a contradiction, so it must in fact every tuple consisting entirely of elements of $B$ and $x$ must be contained in $X_{i}$.
\par Finally, we come to the |Join| constructor. First, note that for the join node of a tree decomposition, the bags $X_{i}=X_{j}$ for the nodes $i , j$ being joined, but the $B_{i}, B_{j}$, the sets of all elements in all bags below $i$ and $j$ respectively, may not necessarily be the same. In fact, they likely are not. The |Join| constructor in Agda requires two proofs, (written as three since we did not make equality for sets): that $X=B_{i} \cap B_{j}$, and that for all relation-tuples in $B_{i} \cup B_{j}$, either the entire relation tuple is in $B_{i}$ or it is in $B_{j}$. In other words, there is no relation tuple in $B_{i} \cup B_{j}$ that contains elements from both $B_{i}\setminus B_{j}$ and $B_{j}\setminus B_{i}$.
...
Suppose $\exists b \in B_{j} \cap B_{k}$ such that  $b \notin X_{i}$. Now, since $b \in B_{j}$, there is some node $m$ at or below $j$ such that $b$ is in $X_{m}$. And, since $b \in B_{k}$, there is some node $l$ at or below $k$ such that $b$ is in $X_{l}$. Since node $i$ joins nodes $j$ and $k$, there is a path between $m$ and $l$ through node $i$. Therefore, by condition three, $X_{m} \cap X_{l} \subset X_{i}$. So $b$ must be in $X_{i}$. However, we assumed the opposite, so we have reached a contradiction. Therefore  $B_{j} \cap B_{k} \subset X_{i}$.
Putting these two proofs together, we see it must be that $B_{j} \cap B_{k} = X_{i}$.
\par The second proof is necessary to preserve the second and third condition of tree decompositions. Suppose that there exists some relation tuple $r \in R^{\mathds{A}} $ such that $r \in B_{j} \cup B_{k}$, and $r \notin B_{j}$ and $r \notin B_{k}$. That is, there exists some $b_{j} \in r$ such that  $ b_{j} \in B_{j} \setminus B_{k}$ and some $b_{k} \in r$ such that $b_{k} \in B_{k} \setminus B_{j}$. Then by condition two of tree decompositions, there must be some node $m$ in the tree decomposition such that $r \in X_{m}$. However, since $r$ contains elements not in $B_{j}$ and elements not in $B_{k}$, node $m$ must be above both $j$ and $k$. And since $r \notin B_{j} \cap B_{k} = X_{i}$, $ r \notin X_{i}$ as well. So $m$ must be above $i$. However, there is some node $a$ below node $j$ such that $b_{j} \in$ the bag $ X_{a}$. And, there must be a path between node $a$ and node $m$, and it must go through node $i$, since $i$ is the only connection between $B_{j}$ and $B_{k}$. Therefore, $b_{j} \in X_{a} \cap X_{m} \subset X_{i}$. A symmetric argument can be used to show that $b_{k} \in X_{i}$. Therefore, $b_{j}, b_{k} \in X_{i} = B_{j} \cap B_{k}$. However, we assumed this was not the case; therefore we have reached a contradiction, and so it must be that for all relation-tuples in $B_{i} \cup B_{j}$, either the entire relation tuple is in $B_{i}$ or it is in $B_{j}$.
\par Now we run through a proof of that fact that for all nodes $i$ of a |TreeDecomp|, $B= \bigcup_{n \text{ at or below } i} X_{n}$. We do this by showing that this property holds at each constructor.
\par We prove by induction. For the base case, we have that if node $i$ is an |Empty|, then it has $\bigcup_{n \text{ at or below } i} X_{n} = \emptyset$ since the bag at a leaf is empty by definition, and there are also no nodes below a leaf. Then since $B=\emptyset$ as well, we have $\bigcup_{n\text{ at or below } i} X_{n} = B$, as desired.
\par Now we run through our inductive steps, one for each constructor. Consider a node $i$. Firstly, suppose $i$ is a |Delete| node. By definition, we know that the node directly below $i$, call it $i-1$, had $X_{i-1} = X_{i} \cup x$ for some $x$. And, we know that $B_{i-1}=B_{i}$. By our inductive hypothesis, we have that $B_{i-1}= \bigcup_{n \text{ at or below } i-1} X_{n}$. Note that $\bigcup_{n \text{ at or below } i} X_{n} = (\bigcup_{n \text{ at or below } i-1} X_{n}) \cup X_{i} $. But since $X_{i-1} = X_{i} \cup x$, then $X_{i} \subset X_{i-1}$, $X_{i}$ brings in nothing new. So $\bigcup_{n\text{ at or below } i} X_{n} = \bigcup_{n \text{ at or below } i-1} = B_{i}$, as desired.
\par Suppose $i$ is an |Introduce| node. Then by definition, $X_{i}= X_{i-1} \cup {x}$ for some $x \notin B_{i-1}$, and $B_{i}= B_{i-1} \cup {x}$. By induction, we have that $B_{i-1} = \bigcup_{n \text{ at or below } i-1} X_{n}$. Note that $\bigcup_{n \text{ at or below } i} X_{n}= \bigcup_{n \text{ at or below } i-1} X_{n} \cup X_{i}$. But note that this is simply $B_{i-1} \cup {x}= B_{i}$, as desired.
\par Suppose $i$ is a |Join| node; suppose it joins nodes $j$ and $k$. By definition, we have $B_{i}=B_{j} \cup B_{k}$, and we have $X=B_{j} \cap B_{k}$. By the inductive hypothesis, we have that $B_{j}= \bigcup_{n \text{ at or below } j} X_{n}$, and $B_{k}= \bigcup_{n \text{ at or below } k} X_{n}$. Note that $\bigcup_{n \text{ at or below } i} X_{n} = B_{j} \cup B_{k} \cup X_{i}$. However, since $X=B_{j} \cap B_{k}$, this is simply $B_{j} \cup B_{k} $. Therefore, $\bigcup_{n \text{ at or below } i} X_{n} =B_{j} \cup B_{k}= B_{i}$, as desired.
\section{MSO Formulas}
...
\end{code}
\par Our encoding of formulas Agda follows the definition above pretty exactly: there is one constructor for every case in the definition, plus an extra constructor for the special formulas |⊤, ⊥| which correspond to a notion that will become clear when we define games in a later section. The first constructor corresponds to the case in the mathematical definition where we have $\phi \in \{ \exists c \psi, \forall c \psi \}$. It takes a $\phi \in MSO(\Sigma \cup {c})$, i.e. the $c$-expansion of $\Sigma$ of for some constant $c$, and turns it into a $\Sigma$ formula.  The second constructor corresponds to the case in the mathematical definition where $\phi \in MSO(\Sigma \cup {R})$, i.e. the $R$-expansion of $\Sigma$ of for some relation $R$. This case does a similar thing to what the first did, but with relations, except it makes the restriction that the added relation can only take one argument, i.e. it must be a set. This is done in the code by specifying that the $R$ in $MSO(\Sigma \cup {R})$ does not have an arbitrary list of argument types |τs| but rather a one-element list of argument types: |r| (the relation) has type |(τ :: [])|.  The third constructor corresponds to creating a formula $\phi \wedge \psi$ or $\phi \vee \psi$ out of two formulas $\psi$ and $\phi$, as in the definition. Then, we add the special formulas corresponding to true and false, i.e. $\top$ and $\perp$.
\par Now we come to the most interesting case for the code, the atomic relation formulas. We introduce a datatype called |Terms| in order to make the definition work. |Terms| has two constructors, |[]| (empty) and |,t| (add one term). In the add one term case, we see that |Terms Σ τs| makes sure that there are symbols in |Σ| that have type |τ| for all |τ| $\in$ |τs|. In other words: recall that a |Signature| |Σ| is a list of symbols, or placeholders, each of a certain type $\tau$. |Terms| ensures that there are actually placeholders in |Σ| that want elements of type |τ| for all |τ| $\in$ |τs|. Then the atomic formula constructor of |Formula| takes a tuple of set elements, |r| of type |τs|, along with evidence that there is a relation in |Σ| that  takes tuples of type |τs|, and out of these elements, creates a formula. This ensures that $R(a_{1}...a_{n})$ is formula of |Σ| only if $R\subseteq \{ \tau_{1}...\tau_{n} \}$ and $a_{1}...a_{n}$ has type $\tau_{1}...\tau_{n}$.
\par Note that we do not create a not constructor. Instead, we have a function |*(φ)| that takes a formula |φ| to its negation, i.e. the dual.
...
\section[Relating Structures]{Relating Structures: Isomorphisms, Compatibility, Union}
\subsection {Isomorphisms of Structures}
\par Now that we understand what structures are, we need to define a notion of isomorphism between structures. The mathematical definition for isomorphisms between structures, taken again from Kneis et al.\cite{kneis}, is as follows:
...
\item $\forall p$-ary relation symbols $R$ in $\Sigma$ and $a_{1}, a_{2}, ... a_{p} \in A$,  \\ $(a_{1}, a_{2}, ... a_{p}) \in R_{\mathds{A}}$ if and only if $(h(a_{1}), h(a_{2}), ... h(a_{p})) \in R_{\mathds{B}}$.
\end{itemize}
\par In order to formalize this notion of isomorphism, which we call |iso|, in Agda, we make a helper function, |preserves|, which defines a function |f| that satisfies the conditions of $h$ above--except the condition that |f| is a bijection. Then our definition of |iso| states that there is an isomorphism between two structures |A1| and |A2| is a function |f| with the evidence that |preserves A1 A2 f| holds, and also |f| has an appropriate inverse. This ensures that |iso A1 A2| holds only if there is a function |f| between them that satisfies all the conditions of $h$, as desired.
\begin{code}
...
preserves (A1 , s1 ,rs U1) (A2 , s2 ,rs U2)  f = Σ (λ (p : preserves (A1 , s1) (A2 , s2)  f)
                                                       → (v : IndividsS A1 _ )
                                                       → (U1 v → U2 (mapIndividS f v))
                                                       × U2 (mapIndividS f v) → U1 v)
\end{code}
\par Let's see how the Agda definition |preserves| corresponds to the definition of isomorphism above, except for the condition that $h$ (|f|) is a bijection.
 We prove this by induction over the signature $\Sigma$, which corresponds to the proof in the code: each condition in the definition above corresponds to one (or two) cases in the code.
...
\par Consider the cases in the code where the last symbol in the |Signature| is a constant symbol, but one structure interprets that symbol as $nil$ and the other does not. (|preserves (A1 , s1 ,is x) (A2 , s2 ,none)  f = Void| and |preserves (A1 , s1 ,none) (A2 , s2 ,is x) f = Void| in the code.) |Void| means false in Agda, so these cases in the code say that |f| cannot exist between two such structures. This corresponds to the part of the mathematical definition which stipulates $\forall$ constants $c \in \Sigma, c^{\mathds{A}}=nil$ if and only if $c^{\mathds{B}}$=$nil$.
\par Consider the case in the code where the last symbol in the |Signature| is a constant symbol, and both structures interpret that symbol as elements of the underlying set. (|preserves (A1 , s1 ,is x) (A2 , s2 ,is xx)  f|  = \\  |preserves (A1 , s1) (A2 , s2) f × (fst (f x) == fst xx)|) This case corresponds to the part of the mathematical definition that stipulates $h(c^{\mathds{A}})=c^{\mathds{B}}$ for all constants $c \in \Sigma$. We see this because |preserves| is a recursive call along with a proof that |(fst (f x) == fst xx)|, i.e. $h(c^{\mathds{A}_{1}})=c^{\mathds{A}_{2}}$ for the last symbol in the |Signature|. The recursive call ensures that $h(c^{\mathds{A}_{1}})=c^{\mathds{A}_{2}}$ for all symbol interpretations in |(A1 , s1)| and |(A2 , s2)|, and the proof |(fst (f x) == fst xx)| ensures that the condition holds for the last symbol in the |Signature|.
\par Now consider the case in the code where the last symbol in each |Signature| is a relation tuple. In this case the code requires a proof that that the third condition of the mathematical definition holds for the relation tuples |U1| $= R^{\mathds{A}1}_{i}$ and  |U2|$= R^{\mathds{A}2}_{i}$, so it clearly corresponds to the third part of the mathematical definition of isomorphism. Thus, all requirements in the mathematical definition are mapped to different parts of the code.
...
\end{code}
\par Finally, we get to the actual Agda definition for an isomorphism between two structures $\mathds{A}1$ and $\mathds{A}2$. |iso| in Agda is a product type of the function |f| between set elements of |A1| and |A2| such that |preserves A1 A2 f|, i.e. that |f| satisfies all the conditions that |preserves| outlines; and a proof that |f| has an inverse. (This is the |IsEquiv| function.) So, we see that the Agda construction of isomorphism satisfies all the necessary properties. The helper function, |preserves|, ensures that  $\forall$ constants $c \in \Sigma, c^{\mathds{A}}$=$nil$ if and only if $c^{\mathds{B}}$=$nil$ by returning void when this condition is not satisfied; and it ensures $f(c^{\mathds{A}})=c^{\mathds{B}}$ for all constants $c \in \Sigma$ and $\forall p$-ary relation symbols $R$ in $\Sigma$ and $a_{1}, a_{2}, ... a{p} \in A$, $(a_{1}, a_{2}, ... a_{p}) \in R_{\mathds{A}}$ if and only if $(f(a_{1}), f(a_{2}), ... f(a_{p})) \in R_{\mathds{B}}$ by proofs it requires of the function |f| upon the |IndividS| and relation expansions.  And |iso| adds the condition that |f| must be a bijection.
...
\end{itemize}
\section{Games}
\par Kneis et al. use games, instead of automata, or fly-automata, or another method, to determine if a given MSO sentence holds on a given structure\cite{kneis}. Model checking games, or $MC$ games, are based off of pebble games, which we define below.

\begin{definition}{Pebble Game\cite{kneis} \textit{Taken from Kneis et al.}}
...
\end{itemize}
\par A game $G$ is said to be determined if either one of the players has a winning strategy, otherwise $G$ is undetermined. The game returns either some indication either Player 1 or Player 2 has a winning strategy, or a proof that the game is a draw.
\end{definition}
\subsection{Model Checking Games}
\par $MC$ games are pebble games specific to a given formula and structure. In the case of $MC$ games, we call one player the falsifier, who wants to show that the formula is false on the given structure, and one player the verifier, who wants to show that the formula is true on the given structure.
\begin{definition}{Model Checking Game\cite{kneis} \textit{Taken from Kneis et al.}}
\par A model checking game $MC(\mathds{A}, \phi) =  (P, M, P_{f}, P_{v}, p_{0})$, over a $\Sigma$-structure $\mathds{A}$ that fully interprets $\Sigma$, and $\phi \in MSO(\Sigma)$, is defined by induction over $\phi$ as follows: we define $p_{0} = (\mathds{A}[\overline{c}^\mathds{A}], \phi)$, where $\overline{c}$ = {all constants $c \in \Sigma$}.
\par If $\phi$ is an atomic formula, i.e. $R(a_{1}, a_{2}, ... a_{p})$ or $\neg R(a_{1}, a_{2}, ... a_{p})$ for some $R \in \Sigma, (a_{1}, a_{2}, ... a_{p}) \in A^{p}$, then $MC(\mathds{A}, \phi) =  (p_{0}, \emptyset, P_{f}, P_{v}, p_{0})$, and $p_{0} \in P_{f}$ if the atomic formula is true (which will make $ P_{f}$, the falsifier, lose) and $p_{0} \in P_{v}$ if the atomic formula is false (which will make $ P_{v}$, the verifier, lose).
...
\par The Agda formulation of $MC$ games is an operation on structures and formulas we call  |⊩c|, or "closed truth". Note that instead of |⊩c| being a game, we simply say that for a |Structure| |(A , SA)| and a formula |φ|,  |(A , SA) ⊩c φ| holds if the verifier of an $MC$ game $MC((A, SA), \phi)$ has a winning strategy. If the falsifier has a winning strategy,|(A , SA) ⊩c φ| fails, and |(A , SA) ⊩c (φ)*| succeeds. This is how we take the game-theoretic element out of the algorithm; we simply model an object very similar to the definition of game given in the code. Other than this, however, the code, below, follows the definition of $MC$ games pretty exactly.
\par We have to trudge through some background code before we can give the definition of |⊩c| directly. We define three helper functions, |Value|, |get|, and |gets|, which help us pick out actual elements from the underlying set to fill our symbols with when needed, that is,  when we need to check that $(c^{\mathds{A}}_{1}, ...c^{\mathds{A}}_{1}) \in R^{\mathds{A}}$ for some atomic formula $R(c_{1}...c_{m})$, or that $(c^{\mathds{A}}_{1}, ...c^{\mathds{A}}_{1}) \notin R^{\mathds{A}}$ for $\neg R(c_{1}...c_{m})$, as the definition for $MC$ games specifies.
\begin{code}
...
\end{code}
\par Now, |get| takes a |SigThing| (symbol) in a |Signature|, a closed structure on that same |Signature|, and returns one of the outputs of |Value|, listed above. The |i0| cases are when the |SigThing| |st| we are looking for is the last element of the |Signature|, and the |iS i1| cases are when we have to look further within the |Signature|, |Σ|, for |st|.
\begin{code}
  gets : ∀ {Σ} {τs : List Tp}
       → Terms Σ τs
       → (A : Structure Closed Σ)
...
\par Finally, we can come to our code for closed truth. |⊩c| requires a $\Sigma$-structure $\mathds{A}$ (i.e. |(A , SA)|) and $\Sigma$-formula $\phi$.  The input $\Sigma$-structure needs to be closed (recall the datatype OC defined way above: this means, as is required in the definition of $MC$ games, that no constants in $\Sigma$ are interpreted as $nil$ in $\mathds{A}$).
\begin{code}
...
\end{code}
\par We now show that |(A , SA) ⊩c φ| holds if the verifier of an $MC$ game $MC((A, SA), \phi)$ has a winning strategy. We start from the trivial cases: |A ⊩c ⊤| and |A ⊩c ⊥|. Recall that |⊤| is the special game for which the verifier always has a winning strategy, and |⊥| is a similar thing for the falsifier. In our code, |A ⊩c ⊤| is defined to be |Unit|, which is equivalent to true in our code, and |A ⊩c ⊥| is set to |Void|, which means that |A ⊩c ⊥| is false, it does not hold. So we see on these simple cases, |⊩c| holds only if the verifier has a winning strategy.
\par Now we move onto existentials and universal formulas over constants. If $\phi = \forall c \psi$, for some constant symbol $c$, then in order for |(A , SA) ⊩c φ|, |(A , (SA ,is a)) ⊩c φ| needs to hold for all $a \in A$ (for all |a : IndividS A τ|). This corresponds to the fact that in order for the verifier to have a winning strategy on $MC(\mathds{A},\forall i \psi)$ there needs to be a winning strategy on $MC(\mathds{A_{a}},\psi)$ for all $a \in A$, where $\mathds{A_{a}}$ is the $(\Sigma, c)$ expansion structure of $\mathds{A}$. (Since on universal formulas, the falsifier would go first.)
\par If $\phi = \exists c \psi$, for some constant symbol $c$, then in order for |(A , SA) ⊩c φ|, there needs to exist some $a \in A$, i.e. some |a : IndividS A τ|, such that  |(A , (SA ,is a)) ⊩c φ|. (The |Σ \ ((a...) → | in the code means "there exists an $a$ such that"). This, in turn, corresponds to the fact that in order for the verifier to have a winning strategy on $MC(\mathds{A},\exists c \psi)$, there needs to be at least one $a \in A$, one next move, such that the verifier has a winning strategy on the subgame $MC(\mathds{A_{a}},\psi)$. A very similar explanation works with the cases $\phi \in  {\forall R \psi, \exists R \psi}$ where $R$ is written in the code as |p|.
\par Now consider the case where the formula is atomic, i.e. $R(c_{1}, c_{2}, ... c_{p})$ or $\neg R(c_{1}, c_{2}, ... c{p})$ for some $R \in \Sigma$. In the first case, in order for the verifier to win, the formula needs to be true, i.e. that $(c^{\mathds{A}}_{1}, ...c^{\mathds{A}}_{1}) \in R^{\mathds{A}}$. And, in order for the verifier to win, this step needs to be the turn of the falsifier, so that the falsifier gets trapped. Since we have done away with the notion of playing a game, we just keep the condition that the formula is true, i.e. $(c^{\mathds{A}}_{1}, ...c^{\mathds{A}}_{1}) \in R^{\mathds{A}}$. The corresponding constructor (|R rel xs|) in the code is exactly a proof that $(c^{\mathds{A}}_{1}, ...c^{\mathds{A}}_{1}) \in R^{\mathds{A}}$. Firstly, |rel| is a symbol that corresponds to a relation and |xs| corresponds to the tuple of nullary constants $(c_{1}, ... c_{m})$.
|gets xs A| is an |IndividsS|, i.e. a list of set elements of |A| which interpret the constant symbols specified in |xs|, and |get rel A (gets xs A)| checks to make sure that this list of set elements is actually inside |rel|, the interpretation of the relation symbol $R$ in |A|. So, |get rel A (gets xs A)| holds only if, indeed,  $(c^{\mathds{A}}_{1}, ...c^{\mathds{A}}_{1}) \in R^{\mathds{A}}$, and so |A ⊩c (R rel xs)| only holds if this is true. A similar argument works for the negation case, but with the constructor only holding if the elements are not in $R$.
\par In the case where $\phi = \psi1 \wedge \psi2$, the falsifier would go first, so the verifier needs winning strategies on both subgames, i.e. |A ⊩c (φ1 ∧ φ2)| requires that both |A ⊩c φ1| and |A ⊩c φ2|.
\subsection{EMC Games}
\par Model checking games are all well and good, but some structures have $nil$s in them, and $MC$ games do not accept that. More pointedly, $MC$ games on their own do not possess the qualities that we need them to have in order for them to be useful in giving an algorithm for Courcelle's Theorem. Recall our explanation of the overall algorithm from Chapter 4. Essentially, in order to prevent exponential blow up for the algorithm at every quantifier (as occurs on the automata implementation of Courcelle's Theorem), we never want to check a quantifier over the entire set of nodes and edges in a graph. Instead of looking at the whole graph at once, the algorithm in Kneis et al. looks at it in pieces: for a structure $\mathds{A}$, the pieces are the restrictions $\mathds{A}[A_{i}]$ associated with each node of the tree decomposition. (Recall that for a node $i$ of a tree decomposition, we let $A_{i}=\bigcup_{j \text{ at or below } i} X_{j}$, and $\mathds{A}[A_{i}]$ is the restriction of $\mathds{A}$ to that subset.) The algorithm that we use to prove Courcelle's theorem traverses the given tree decomposition bottom up, checking to see if the formula holds on $\mathds{A}[A_{i}]$ for each node, starting at the empty leaves, and adding a little bit of the set at a time.
\par It may be hard to believe that this algorithm returns the correct result. First of all, the subsets $A_{i}$ do not fully interpret the signature, and secondly, how do we know that if there is a winning strategy on one of these smaller games, it propagates to a winning strategy on the overall game? To satisfy the first point, we need games that can handle $nil$ assignments, which is the first handy property of Extended Model Checking games, or $EMC$ games. Additionally, consider the idea that, if you can satisfy an equation with a structure full of $nil$ assignments, you can satisfy that equation on a structure with any interpretations for those $nil$ assignments, because the elements that are interpreted as $nil$ are not doing anything consequential if they don't prevent the MSO formula from being satisfied. This is because if the constants were important, if some $c^{\mathds{A}}_{i}$ were interpreted as $nil$ came up in some atomic formula $R(c_{1}....c_{n})$, at the end of the game, then the game would be undefined by definition. But if the game comes back true, this means that that particular constant had nothing to do with the formula at all, since this situation never arose. So having a winning strategy on some game made with a restricted structure, chock full of $nil$s as it may be, should propagate to having a winning strategy on the full game, since giving specific values to constants that weren't affecting the formula anyway is not going to change anything. But even with this fact, we still have to come up with a way of propagating the winning strategy upward. We have to find a method of seamlessly transitioning from a game on a restricted structure to a game on the a larger restriction, or the whole structure.
\par Consider the fact that for each node $i$ in a tree-decomposition $T$ of a structure $\mathds{A}$, while we have  $A_{i}=\bigcup_{j \text{ at or below } i} X_{j}$, we can also consider $A \setminus A_{i} := B_{i}$, the set of everything in A not yet taken into account by the tree decomposition. This set is often called, illustratively, the future of $A_{i}$. Just as we can restrict $\mathds{A}[A_{i}]$, we can define a structure $\mathds{A}[B_{i}]$. Recalling our notion of the union of two structures from earlier, see that at any node $i$, $\mathds{A}=\mathds{A}[A_{i}] \cup \mathds{A}[B_{i}]$. So, the condition we want is that, if we have a winning strategy on a game for $\mathds{A}[A_{i}]$ and $\phi$, then we want to propagate this to a winning strategy on $\mathds{A}[A_{i}] \cup \mathds{A}[B_{i}] $ and $\phi$. Thankfully, Kneis et al. created $EMC$s with the property that are well-defined on taking the union of structures (provided the structures are compatible)\cite{kneis}.
\par To summarize, Kneis et al. developed another form of model checking game, which does allow structures with $nil$s, and is well defined over taking unions of compatible structures, i.e. if there is a winning strategy for a game over $\mathds{A}$ and $\phi$, that translates to a winning strategy on $\mathds{A} \cup \mathds{B}$ and $\phi$ for two compatible structures $\mathds{A}$ and $\mathds{B}$.  In the Kneis et al. paper, an $EMC$ is defined to keep track of what bag of the tree decomposition it is on. We found this unnecessary in the code, but provide the original definition here for clarity.
\begin{definition}{Extended Model Checking Game\cite{kneis} \textit{Taken from Kneis et al.}} [An extended model checking game $EMC(\mathds{A}, X, \phi) = (P, M, P_{f}, P_{v}, p_{0})$, over a $\Sigma$-structure $\mathds{A}$, a set X $\subset$ A, and $\phi \in MSO(\Sigma)$, is defined over induction over the structure of $\phi$ as follows. Let $p_{0} = (\mathds{A}[\overline{c}^\mathds{A} \cup X], \phi)$, where again $\overline{c}$ = {all constants $c \in \Sigma$}. If $\phi$ is an atomic formula, then $EMC(\mathds{A}, X, \phi) = ({p_{0}}, \emptyset, P_{f}, P_{v}, p_{0})$, where $p_{0} \in P_{f} $ if and only if there are no $nil$s in the relation tuple and the formula is true; i.e. if the formula is $R(c_{1}, c_{2}, ... c_{p})$ or $\neg R(c_{1}, c_{2}, ... c{p})$ for some $R \in \Sigma$, then each $c^{\mathds{A}}_{i}$ in $(c^{\mathds{A}}_{1}, ...c^{\mathds{A}}_{1})$ must not be $nil$, and the formula is actually true, i.e.  $(c^{\mathds{A}}_{1}, ...c^{\mathds{A}}_{1}) \in R^{\mathds{A}}$. Again, as explained for $MC$ games, this will make the verifier win. Alternatively, $p_{0} \in P_{v}$ if the tuple $(c_{1}, c_{2}, ... c_{p})$ is fully interpreted, and the atomic formula is false (which will make $ P_{v}$, the verifier, lose).
...
\end{itemize}
\end{definition}
\par The way that we model $EMC$ games in the code is through a datatype |⊩o|, or open truth. Similar to our definition of closed truth above, we do away with the notion of a game and players, and just outline |⊩o| so that for a structure $\mathds{A}$ and a formula $\phi$,|A ⊩o φ| if and only if there is a winning strategy for the verifier on an $EMC$ game $EMC(\mathds{A}, X, \phi)$. Before we can get to this definition, however, we have to explain some of the code leading up to it.
\begin{code}
 ValueOpen : Subset → SigThing → Type
  ...
  ... | _ | _ = None
\end{code}
\par Similar to our code for closed truth, we define |ValueOpen|, |getOpen|, and |getsOpen|, which perform almost the same functions that |Value|, |get|, and |gets| do, except that they all take into account the fact that for open truth and $EMC$ games, we could be working with open structures, which may contain $nil$s. So, |ValueOpen|, instead of returning an set element that is an interpretation when given a nullary constant, gives either |none| (for $nil$) or |Some v| (for some set element). This propagates through |getOpen| and |getsOpen|: |getOpen| looks up the set element used to interpret the symbol |st| in the |Signature|, failing if it finds none. |getsOpen|, when given |Terms| (a tuple of constant symbols) and a structure, returns a tuple of elements that interpret the constant symbols--except that, if even one of the symbols is not interpreted, it returns a |none| value instead of a tuple of set elements.
...
\end{code}
\par Based on these different types of formulae, we defined a notion of a branch, of which there are universal branches and existential branches. Think of a branch as an edge between two nodes in a game: it keeps track of which formula and structure the game is are traveling from and towards. (For as the algorithm goes through a game, the formula shrinks and the structure grows). So if there is a node (i.e. a position in the definition above)
$(\mathds{A}, \phi)$ where one possible next move is to $(\mathds{A} ', \phi ')$, then there will be a branch |branch A φ A' φ'| too in the game to show that. We will see further below in our definition of game.
Included below is only the universal branch code, as the existential branch code is quite similar. |Ubranch| has a different constructor for all the different kinds of branches required for different formulas. For an and node, $\phi = \psi_{1} \wedge \psi_{2}$, we need a branch going to each of $\psi_{1}$ and $\psi_{2}$, and this is what ufstb and usndb are. If  $\phi = \forall c \psi$, we need a branch $\mathds{A} \rightarrow \mathds{A}'$ where $\mathds{A}'$ is a $c$-expansion of $\mathds{A}$ with $a$ interpreted as $c$. Finally, in the case that $\phi = \forall R \psi$, |ubranch| creates a branch for all tuples $r$ of elements in $A$ to interpret $R$.
\begin{code}
data ubranch {Σ1 : Signature} : ∀ {Σ2 : Signature} {oc1 oc2}
                                  (A1 : Structure oc1 Σ1) (φ1 : Formula Σ1)
                                  (A2 : StructureS oc2 (fst A1) Σ2) (φ2 : Formula Σ2) → Type where
    ufstb : ∀ {oc} {A1 : Structure oc Σ1} → {φ1 φ2 : Formula Σ1}
                       → ubranch A1 (φ1 ∧ φ2) (snd A1) φ1
    usndb : ∀ {oc} {A1 : Structure oc Σ1} → {φ1 φ2 : Formula Σ1}
                       → ubranch A1 (φ1 ∧ φ2) (snd A1) φ2
    uforallb : ∀ {oc} {τ} {A1 : Structure oc Σ1}  → {φ : Formula (i τ :: Σ1 )}
                 (a : IndividS (fst A1) τ)
               → ubranch A1 (∀i τ φ) ( (snd A1) ,is a) φ
    uforallnil : ∀ {oc} {τ} {A1 : Structure oc Σ1}
                → {φ : Formula (i τ :: Σ1 )}
                → ubranch A1 (∀i τ φ) ((snd A1) ,none) φ
    uforallr : ∀ {oc} {τ} {A1 : Structure oc Σ1} → {φ : Formula (r (τ :: []) :: Σ1 )}
               (r : IndividsS (fst A1) (τ :: []) → Type)
...
\begin{code}
data branch {Σ1 : Signature} : ∀ { Σ2 : Signature} {oc1 oc2}
                               (A1 : Structure oc1 Σ1) (φ1 : Formula Σ1)
                               (A2 : StructureS oc2 (fst A1)  Σ2)
                               (φ2 : Formula Σ2) → Type where
...
\end{code}
\par Finally, we can look at the code for open truth, our Agda encoding of an $EMC$ game. It has four constructors: |provesu| (for proving universal formulas), |provese| (for proving existential formulas), |provesbase| and |provesnotbase| (both for atomic formulas). As noted when we explained closed truth and $MC$ games, we take the game element out of the $EMC$ game. We model $EMC$ games with a datatype |⊩o| such that for a structure $\mathds{A}$ and a formula $\phi$, |A ⊩o φ| if and only if there is a winning strategy for the verifier on an $EMC$ game $EMC(\mathds{A}, X, \phi)$.
\begin{code}
data _⊩o_ {oc : _} {Σ : _} (A : Structure oc Σ) : Formula Σ → Type where
    provesu : {φ : Formula Σ}
            → isU φ
            → (∀ {Σ' oc'} {A' : StructureS oc' (fst A) Σ'} {φ'}
            → ubranch A φ A' φ' → (fst A , A') ⊩o φ')
            → A ⊩o φ
    provese : ∀ {Σ' oc'} {A' : StructureS oc' (fst A) Σ'} {φ : Formula Σ} {φ'}
            ...
            → (fst A , A') ⊩o φ'
            → A ⊩o φ
    provesbase : ∀ {τs} {rel} {xs : Terms _ τs} vs
                → ((getsOpen xs A) == (Some vs))
                → (getOpen rel A) (vs) → A ⊩o (R rel xs)
    provesnotbase : ∀ {τs} {rel} {xs : Terms _ τs}
                  → (vs : _) → ((getsOpen xs A == (Some vs)))
                  → ((getOpen rel A) (vs) → Void)
                  →  A ⊩o (¬R rel xs)
\end{code}
\par We show that |⊩o| holds exactly when a verifier has a winning strategy on an $EMC$ game. Consider the |provesbase| case in the code, which corresponds to the place in the $EMC$ definition where the formula is $R(c_{1}, ...c_{m})$ for some relation symbol $R$ in $\Sigma$. In the mathematical definition, in order for the verifier to win in this case, the atomic formula needs to be correct, and it should be the falsifier's turn. Since we have taken the notion of players in the game out, we only need the proof that the atomic formula is true. And this is exactly what the |provesbase| constructor is: a proof that the formula holds. Given a tuple, |vs|, of elements in the underlying set (i.e. |vs : IndividsS A τ s|) |((getsOpen xs A) == (Some vs))| is a proof that the |xs|, i.e. the tuple $(c_{1}, ...c_{m})$ in the formula $R(c_{1}, ...c_{m})$ is actually interpreted as this tuple of set elements |vs|, i.e. there are no $nil$s. We can see this because |getsOpen| takes a list of constant symbols (|Terms Σ τs|) and a structure, and makes sure that each of those symbols is interpreted on that structure (i.e. returns |Some v| in |ValueOpen|). Then, |(getOpen rel A) (vs)| checks to make sure that |vs|, the actual tuple of elements of |A| (the |IndividsS-A-τs|), is actually in $R^{\mathds{A}}$. We see this because |getOpen| will return a |ValueOpen| for this relation |rel| on A, i.e. a function that checks if a given tuple is in a given relation, and so in |(getOpen rel A) (vs)| we are using that given function to check if |vs| $\in R^{\mathds{A}}$. A similar proof will show that the |provesnotbase| constructor is a proof that $\neg R(c_{1}, ...c_{m})$ holds, i.e. $(c^{\mathds{A}}_{1}, ...c^{\mathds{A}}_{m}) \notin R^{\mathds{A}}$. The only real difference is that in the |provesnotbase| constructor, the |((getOpen rel A) (vs) → Void)| signifies that the tuple |vs| is not in $R^{\mathds{A}}$.
...
\begin{enumerate}
\item The formula is universal (|IsU φ|)
\item For all $\Sigma '$-structures $\mathds{A}'$ with the same underlying set as $\mathds{A}$, and all $\Sigma '$-formulas $\phi '$, if there's a branch (a |ubranch|, since we're in the universal case) between $\mathds{A}$, $\phi$ and $\mathds{A}'$ $\phi '$, then $\mathds{A}'$|⊩o|$\phi '$
\end{enumerate}
\par Note that depending on what exact formula $\phi$ is, the |ubranches| from $\phi$ will branch to different  $\mathds{A}'$, $\phi '$-- we don't make a separate constructor for each case. But if, for example, the formula were $\forall c \psi$ for some constant $c$, then there would be a |ubranch| for $\mathds{A_{a}}$ for all $a \in A$, where $\mathds{A_{a}}$ is a $\Sigma , c$ expansion of $\mathds{A}$ with c interpreted as $a$ for all $a \in A$. This corresponds to a winning strategy for the verifier because, since the falsifier gets to choose the next move on a universal quantifier, the verifier needs a winning strategy on every subgame that the falsifier could move to. We interpret this as |A' ⊩o φ'| for every possible $\mathds{A}'$, $\phi '$ we can branch to from $\mathds{A}$, $\phi$.
\par A very similar explanation to the one above works for the |provese| constructor. In this case, we just need to show that the formula is existential, and that |A' ⊩o φ'| for one possible $\mathds{A}'$, $\phi '$ we can branch to from $\mathds{A}$, $\phi$. This corresponds to the verifier having a winning strategy on a corresponding $EMC$ game because since the verifer gets to choose the next move on existentials, we only need to find one subgame that the verifier can win, i.e. where |A' ⊩o φ'|. Again, the specific subgames that are checked depend on the |ebranch| constructor. We also have a definition, |⊩o false|, which takes a structure and a formula as before and shows that the formula is false on that structure. This corresponds to a falsifier having a winning strategy on an analogous $EMC$ game, and we define it as a |⊩o| that proves the negation of the formula given.
\section{Reduced Games}
\par As described above, the algorithm used to prove Courcelle's theorem in the Kneis et al.\cite{kneis} paper runs through an entire $EMC$ game on each node of the tree decomposition, and propagates undecided games upward through the tree decomposition until it has enough data to come to a definitive answer as to whether or not the formula holds on the given structure. In order to further reduce the time of the algorithm, we want to be running games as efficiently as possible. In order to do this, we want to keep as few possible next positions (analogously in our code, as few branches) as possible. This way, we have fewer places to move to at each step, and so our game becomes much smaller. We would like to keep track of only the necessary information as we propagate games upward. For example, if we are able to achieve a winning or losing strategy on a game at a given node, we want to exit the algorithm there since we've found out answer. If our game is undecided, we still don't have to keep all information that is available to us: for example, if our formula is universal, we don't have to keep track of all of the different subgames that prove the formula true.
\subsection{Game Tree}
\par With this in mind, we introduce another game-like structure in the Agda code, which will become the underlying type for something called a reduced game. A reduced game will keep track of only the information from any given game that we absolutely need. The explanation for all of this will come later, but because of the way the code is structured, we will introduce a constructor called |⊩s|, which we call a \textit{raw game tree}, now.
\begin{code}
 data _⊩s_ {oc : _} {Σ : _} (A : Structure oc Σ) (φ : Formula Σ) : Type where
    ...
    node : (bs : Branches A φ)
         ...
         → A ⊩s φ
\end{code}
\par A |⊩s| is a game that is either atomic, or has a certain set of branches. However, which branches these are is not specified. That is, it does not require the exact set of outward branches (subgames) that open truth or closed truth require. We enclose the code above. See that a |⊩s| is a datatype with two constructors, leaf and node. The leaf constructor is simply a proof that the formula is atomic, |isR φ|. There are no outward branches, i.e. subgames, here. The node constructor is a product type, consisting of (1) a list of branches (|bs|) from the given structure A and formula $\phi$ (i.e. the game position that has structure A and formula $\phi$), and (2) a proof that for all structure, formula pairs $A', \phi '$ (i.e. positions) that any branch |bi ∈ bs| points toward, |A' ⊩s φ'|. We can also think of (2) as a function that takes a branch |b|, a proof that |b| is in the list of branches of some game tree, and returns a game tree on the structure that |b| branches towards.
\par Note the difference between open truth, closed truth and game trees: as mentioned earlier, there are no specifications on the branches of a game tree---unlike for the open truth and closed truth datatypes, if the formula $\phi = \psi_{1} \wedge \psi_{2}$, there is no requirement in a game tree that there be a branch for both $\psi_{1}$ and $\psi_{2}$. There's just some unspecified list of branches; and what goes in that list will become clear later.
\par Continuing from the discussion above about throwing out unnecessary information in games, how do we decide which next positions (or branches) to keep and which ones to throw away? To help answer this question, we introduce a concept of \textbf{equivalent games}.
\subsection{Position Equivalence}
...
\begin{code}
 BranchBijection : {Σ : Signature} {oc1 oc2 : _} (A1 : Structure oc1 Σ)
                   (A2 : Structure oc2 Σ) (φ : Formula Σ)
                   → Branches A1 φ → Branches A2 φ → Type
 BranchBijection A1 A2 φ branches1 branches2 =
                  ∀ {Σ' φ'} →
                    Equiv (Σ (λ oc' → Σ (λ (A' : StructureS oc' (fst A1) Σ') →
                    Σ \ b → (branchto A' φ' b) ∈ branches1)))
                   (Σ (λ oc' → Σ (λ (A' : StructureS oc' (fst A2) Σ')
                   → Σ \ b → (branchto A' φ' b) ∈ branches2)))

\end{code}
\par |BranchBijection| takes two structures $\mathds{A}_1$, $\mathds{A}_2$, a formula $\phi$, and the list of branches that come out of each structure. |BranchBijection| holds on these elements if there is a bijection between the set of structures $\bigcup_{i}\mathds{A}_{1,i}$ that  $\mathds{A}_{1}$ branches to, and the set of structures $\bigcup_{i} \mathds{A}_{2,i}$ that $\mathds{A}_{2}$ branches to. The type of the result of |BranchBijection| is a product type of a function from $\bigcup_{i}\mathds{A}_{1,i}$ to  $\bigcup_{i} \mathds{A}_{2,i}$, and a proof that that function is a bijection, i.e. an inverse function  $\bigcup_{i} \mathds{A}_{2,i}$ to $\bigcup_{i}\mathds{A}_{1,i}$. (This comes from the |Equiv| in the code; but we won't go into an explanation of that).
...
\begin{code}
 gameEquiv : ∀ {Σ oc1 oc2} (A1 : Structure oc1 Σ) (A2 : Structure oc2 Σ)
             (φ : Formula Σ) → A1 ⊩s φ → A2 ⊩s φ → fixed (fst A1) (fst A2) → Type
 ...
 gameEquiv' A1 A2 φ (node bs1 x1) (node bs2 x2) f =
               Σ (λ (b : BranchBijection A1 A2 φ bs1 bs2) →
                ∀ {Σ' oc'} {A' : StructureS oc' (fst A1) Σ'} {φ'} bi →
                (brnchi : branchto A' φ' bi ∈ bs1) →
                 gameEquiv ((fst A1) , A') ((fst A2) ,
                 (fst (snd (fst b (_ , A' , bi , brnchi))))) φ' (x1 bi brnchi)
                 (x2 _ (snd (snd (snd (fst b (_ , A' , bi , brnchi)))))) f)
\end{code}
\par |gameEquiv'| does most of the work of the recursive calls. It takes the same input as |gameEquiv|, but here we have one case for each of the possible combinations of constructors of the two game trees are made up of. If the game trees are both leaves, there are no subgames (i.e. structures that are branched to) and so no recursive calls are needed, the subgames are equivalent vacuously, and |gameEquiv'| returns |Unit|, or True. If one game tree is a leaf and the other is a node, there is no possible way for them to be equivalent, so |gameEquiv| returns |Void|, or false. Now the interesting case: both game trees are nodes. Then, |gameEquiv' A1 A2 φ (A1 ⊩s φ) (A2 ⊩s φ) X| holds if there exists a |BranchBijection| $b$ between the list of branches in the | (A1 ⊩s φ)| game tree and the list of branches in the | (A2 ⊩s φ)|  game tree, such that for all structures |A'| (with underlying set |A1|) and formulas |φ'| that |A1| and |φ| branches to, |gameEquiv A' b(A') φ' A' ⊩s φ' b(A') ⊩s φ' X| holds. Through mutual recursion, the two functions determine if two games are equivalent, with |gameEquiv| checking the initial positions are equivalent and |gameEquiv’| continuing to send the correct subgames to |gameEquiv| to check for position equivalence.
\par If two games are equivalent, they will have the same outcome\cite{kneis}. Certainly, for each game $G$, we only need to keep one representative of each equivalence class of  $subgames(G$) modulo $\cong$. Additionally, note that for any game $G$, we can discard the subgames where the opposing player, which in our case will always be the falsifier, wins. There is no use in checking a path that is known to fail\cite{kneis}.
\subsection{Reduced Games Definition}
\par With this in mind, we give a definition of a \textit{reduced game}. A reduced $EMC$ game $G$ over a $\Sigma$-structure $\mathds{A}$ and a $\Sigma$-formula $\phi$, $reduce(\mathds{A}, X, \phi)$ is defined by induction as follows:
\begin{itemize}
...
\end{code}
\par |isRed| takes a structure, $\mathds{A}$, a formula $\phi$, a game tree |A ⊩s φ|, and a bag $X$. In the case that the game tree is a leaf, which means it has no outward branches, |isRed| is true vacuously and so the definition for that case is |Unit|, or true. In the case that the game tree |A ⊩s φ| is a node, we have a double product type; there are three requirements appended together. The first one (the code before the first $\times$) requires that for all |StructureS|'s |A'| and |Formulas| |φ'| such that there is a branch  $bj \in bs$ that connects ($\mathds{A}$, $\phi$) $\rightarrow$ ((fst $\mathds{A}$, A'), $\phi$'), there is a reduced game on ((fst $\mathds{A}$, A'), $\phi$'), i.e.   |isRed (fst A , A') φ' (x bj prfbr) fix| holds. In case it is confusing that  |(x bj prfbr)| is miraculously a game tree |(fst A , A') ⊩s φ'|. recall from the definition of |⊩s| that the second argument can be thought of as a function that takes a branch and a proof that that branch is in the list of branches corresponding to some game tree, and returns a game tree on the structure that the branch points to. In the code here, we are calling that second argument |x|, and we see that |bj| is a branch, and |prfbr| is a proof that |bj| is in |bs|, i.e. the list of branches of |A ⊩s φ|. So, |(x bj prfbr)| is a game tree |(fst A , A') ⊩s φ'|.
\par Now, for the next two parts of the product type, the first corresponds to the case where the formula is universal, the other to the case where the formula is existential (the atomic cases will only happen with a leaf.) Let's tackle the first of these two, which corresponds to the formula being universal. The first thing required is a proof that the formula is universal, |isU  φ|. Then we need a proof that for all |StructureS|'s |A'| and formulas |φ'| such that there is a branch |bi| between nodes |(A, φ)| and (|(fst A , A'), φ'|), either
\begin{itemize}
\item there is a proof that branch is a part of the list of branches of |A ⊩s φ|  or
\item Either |((fst A , A')  ⊩o φ')| i.e. the verifier has a winning strategy on the subgame labeled by that structure, formula pair; or
\item  there is another |StructureS|, |A''|, such that
\begin{itemize}
\item there is a branch between nodes |(A, φ)| and (|(fst A , A'), φ'|) ;
\item there is a subgame |(fst A, A'') ⊩s φ'| that is reduced, i.e.  |isRed (fst A , A') φ' gi fi| (where |gi = (fst A, A'') ⊩s φ'| and |fix| = the bag |X|), and importantly, that there is a proof that this subgame is equivalent to |(fst A , A') ⊩s φ'|, i.e.  |gameEquiv (fst A , A') (fst A , A'')| \\ |φ' gi (x bj pfbr2) (lemma1 fix)|.
\end{itemize}
\end{itemize}
...
\par The third part of this product type is very similar. The only difference is as follows. Instead of insisting that for all subgames, either the verifier has a winning strategy on the subgame, there is an isomorphic subgame to it in the list of subgames kept alive, or the subgame is in that list itself; we ask that for all subgames, either the falsifier has a winning strategy (|Either ((fst A , A')  ⊩o φ' false|)) or one of the other options. Again, we see this matches the mathematical definition for reduced games nicely.
...
\end{code}
\par |provesR| takes a structure $\mathds{A}$, a formula $\phi$, a game tree |A ⊩s φ|, and a bag |X|, and gives a proof that there is a game tree |A ⊩s φ| such that |isRed A φ  (A ⊩s) φ| holds.

\chapter[Algorithm and Progress]{Outline of Algorithm and Work in Progress}
\section{Proof of Algorithm}
\par While we will prove our own version of the overall proof of the algorithm with our code, we outline the proof suggested by Kneis et al. here\cite{kneis}. We have extracted this proof from the more complex algorithm Kneis et al. give for a version of Courcelle's Theorem that allows for linear optimization questions in MSO\cite{kneis}. However, in an effort not to reproduce too much of their paper, we simply rely upon several lemmas from the paper by Kneis et al. We reproduce the statement of the lemmas below, but for the proof, please refer to the Kneis et al. paper. Additionally, note that we refer to two algorithms not mentioned yet, \textit{combine} and \textit{combineForget}. These are algorithms laid out in the Kneis et al. paper which we will not reproduce, but please think of them as similar to the agda functions |combineIntro|, |combineJoin|, and |combineForget| which will be described below, after the proof.
\par We show that given a $\Sigma$-structure $\mathds{A}$ that fully interprets $\Sigma$, a $\Sigma$-formula $\phi \in MSO(\Sigma)$, and a tree decomposition $T$ on $\mathds{A}$ and $\phi$, we can return a game that determines whether or not $\mathds{A}$ satisfies $\phi$. The following are the lemmas from the Kneis et al. paper that we use\cite{kneis}:
...
\par Suppose node $i$ is a join node, structure $\mathds{A}[A_{l} \cup A_{r}]$ and formula $\phi$. Then by definition of a join node, there are two tree decompositions \textbf{$T_{l}$} and \textbf{$T_{l}$}, corresponding to the left and right children $i$.  \textbf{$T_{l}$} is over $\mathds{A}[A_{l}]$, $\phi$, where $A_{l}= \bigcup_{\text{i at or below l}} X_{i}$, and \textbf{$T_{l}$} is over $\mathds{A}[A_{r}]$, $\phi$, where $A_{r}= \bigcup_{\text{i at or below r}} X_{i}$. We want to show that \linebreak $combine(reduce(A_{l}, X, \phi)$, $reduce(A_{r}, X, \phi))=reduce(A_{l}\cup A_{r}, X, \phi)$. Recall Lemma 11 defined above; we wish to apply this lemma to prove our proposition.
...
\par Now we show that the rest of the conditions of Lemma 11 hold, namely that $X \subset A_{l}$, $X \subset A_{r}$, and that $X \cap X = A_{l} \cap A_{r}$.  Recall the definition of $A_{i}$ for a node $i$ : $A_{i} = \bigcup_{\text{j at or below i}}X_{j}$ for all bags $X_{j}$ for nodes $j$ equal to or below $i$. So, by definition of  $A_{l}$ and  $A_{r}$, $X \subset A_{l}$ and $X \subset A_{r}$ since $X$ is the bag at nodes $l$ and $r$. We see that $X \cap X = A_{l} \cap A_{r} = X$. Firstly note that $X \subset X = A_{l} \cap A_{r} $ necessarily because the bag of a join node is defined to be $X=X_{r}=X_{l}$, and therefore $X \subset A_{r}$ $X \subset A_{l}$. To see the other containment, recall that by condition three of a tree decomposition, all bags that contain the same element must be connected. If there exists a $y \in A_{l} \cap A_{r}$, $y \notin X$, then this condition would not hold. If $y \in A_{l}$ and $y \in A_{r}$, there must exist some node $l'$ at or below $l$ and a node $r'$ at or below $r$ such that $y \in X_{l'}$ and $y\in X_{r'}$. Then by condition 3 of tree decompositions, there must be a path between these two nodes such that $y$ is contained in every bag of every node on that path. However, the only connection between $l^{'}$ and $r^{'}$ is through the node that connects $r$ and $l$, which has bag $X$, so $y$ must be in $X$. Therefore,  $A_{l} \cap A_{r} = X$ as desired. With these qualifications in place, we can apply Lemma 11 and get that $combine(reduce(A_{l}, X, \phi), reduce(A_{r}, X, \phi))=reduce(A_{l}\cup A_{r}, X, \phi).$
\par Suppose $i$ is an introduce node, over structure $\mathds{A}[A_{j}\cup {x}]$, (where $j$ is the node directly below $i$ and $x \notin A_{j}$) and formula $\phi$. Then by definition of an introduce node, we have a tree decomposition $T_{j}$ at node $j$ where the tree decomposition is over structure $\mathds{A}[A_{j}]$ and $\phi$. We want to show that
\begin{multline*}
combine(reduce(\mathds{A}[A_{j}], X_{j}, \phi), reduce(\mathds{A}[X_{j} \cup {x}], (X_{j}\cup {x}), \phi)) \\ =reduce(A_{j}\cup {x}, X_{j}\cup{x}, \phi)
\end{multline*}
\par We can use Lemma 11 again: we let $X_{1}=X_{j},$ $X_{2}=X_{j}\cup {x},$ $\mathds{A}_{1}=\mathds{A}[A_{j}]$ and $\mathds{A}_{2}=\mathds{A}[X_{j} \cup {x}]$. Again we require that $\mathds{A}[A_{j}]$ and $ \mathds{A}[X_{j} \cup {x}]$ are compatible, $X_{j} \subset A_{j}$, $X_{j}\cup {x} \subset X_{j} \cup {x}$, and that $X_{j} \cap (X_{j} \cup {x}) = A_{j} \cap (X_{j}\cup {x})$. The proof of compatibility for the two structures is exactly the same as the proof above, substituting the relevant subsets. For the rest of the requirements, note that again $X_{j}\in A_{j}$ by definition of $A_{j}$. $X_{j}\cup {x} \subset X_{j} \cup {x}$ since they are the same set. $X_{j} \cap (X_{j} \cup {x})= X_{j}$ since by assumption $x \notin X_{j}$, and $A_{j} \cap (X_{j}\cup {x}) = X_{j}$ as well since by definition $X_{j}\subset A_{j}$, but $x \notin A_{j}$. Therefore $X_{j} \cap (X_{j} \cup {x}) =X_{j} = A_{j} \cap (X_{j}\cup {x})$ as required, so by Lemma 11 we have the desired result that $combine(reduce(\mathds{A}[A_{j}], X_{j}, \phi), reduce(\mathds{A}[X_{j} \cup {x}], (X_{j}\cup {x}), \phi))= \\ reduce(A_{j}\cup {x}, X_{j}\cup{x}, \phi). $
\par Suppose $i$ is a forget node. Then by definition of a forget node, we have a tree decomposition $T_{j}$ at node $j$ directly below $i$ such that $A_{j}=A_{i}$ but $X_{j}=X_{i}\cup {x}$ for some $x \notin X_{i}$. $T_{j}$ is over structure $\mathds{A}[A_{j}]$ and $\phi$. We want to show that $combineForget(reduce(\mathds{A}[A_{i}], (X_{i}\cup {x}), \phi), x)=reduce(\mathds{A}[A_{i}], X_{i}, \phi)$. Then recall Lemma 12, which we will use for this proof. Let $\mathds{A}$=$\mathds{A}[A_{i}]$, $X= X_{i} \cup {x}$, and $x=x$. The lemma requires that $ X_{i} \cup {x} \subset \mathds{A}[A_{i}]$, which is true by construction, and that $x \in X_{i}\cup {x}$, which is also true by construction. So, by Lemma 12, we have the desired result that $combineForget(reduce(\mathds{A}[A_{i}], (X_{i}\cup {x}), \phi), x)=reduce(\mathds{A}[A_{i}], X_{i}, \phi).$
...
\par This proof is fine and dandy, but it does not mention the time bound of the algorithm, which is arguably the entire reason why Courcelle's Theorem is interesting. Unfortunately, proving the time bound is one of the parts of the Courcelle's Theorem that is left for the future of this project. However, we present an intuitive idea of the proof. Note that the brute force $MC$ game algorithm takes as many steps as there are quantifiers or conjunctions in the MSO formula, and that it takes exponential time in the size of the input structure because you have to try every element in the input structure at every quantifier. However, note that the only place we ever apply the brute force $MC$ game on the structure is on games where the structure is restricted to the bag of a given node. But these bags, by definition of a tree decomposition, have size less than or equal to $t$ where $t$ is the treewidth of the input graph $G$. So, each time we run the brute force algorithm, even though the algorithm is exponential in the size of the input structure, it ends up being constant time because the time it takes is not related to the size of the overall input graph, but only the treewidth. The algorithms combine and forget are linear because they only ever work over reduced games, and the of the size of a reduced game is small enough to keep these calculations under FPT linear time as explained in the Kneis et al. paper\cite{kneis}.

\section{Agda Code and Proof}
...
\item that the falsifier has a winning strategy on $EMC$($\mathds{A}$, X, and $\phi$),
\item or a reduced undecided game $reduced(A[B], X, \phi$).
\end{itemize}
The idea of what the proof of this function will look like is that, like the proof above, it will case on the tree decomposition. It will use the functions |combineJoin| and |combineIntro| to take care of combining the recursive calls where the mathematical proof used Lemma 11, and it will use |combineForget| where the mathematical proof used Lemma 12. If |algorithm| returns an undecided reduced game, that game will be sent to |provesRtoClosed|, which is our interpretation of Lemma 13 from the proof above, and if |algorithm| returns a decided $EMC$ game (i.e. an open truth), that result will be sent to |openToClosed| which is our interpretation of another lemma from the paper.
\par We see quite clearly that |openToClosed| takes a structure and a formula, a |⊩o|, i.e. an $EMC$ game on that pair, and then sends it to a |⊩c|, i.e. an $MC$ game, on that pair. |provesRtoClosed|, similarly, takes reduced games to $MC$ games, as lemma 13 does. The general idea of both of these proofs is essentially that if your structure is closed, you can simply take out the branches where constants were interpreted as $nil$ to keep options open for future expansions, (since all possible options are already expressed), and see if the closed $MC$ game left behind has a winning strategy for the verifer. But even more simply, if a game is still undecided even when the entire underlying set is available to the game, then the game is true if it is a universal game and false if it is an existential game. This is because the $nil$ is kept as a sort of open space for more possible interpretations of constants, if more of the underlying set is to be merged in. If a universal game is undecided, it must be because the verifier has a winning strategy on all branches except those with $nil$, because if the falsifier had a winning stategy on even one subgame, the game  would have been terminated. However, if the $nil$ doesn't really exist since all possible interpretations have been explored, this must mean that the formula does work for every possible interpretation and the universal formula is true on the structure. A dual argument works for existential formulas being false.
\begin{code}
...
\begin{code}
   {-EMC -> MC i.e. Lemma 11-}
  openToClosed : ∀ {Σ} → (A : Structure Closed Σ) (φ : Formula Σ)
                       → (otruth : A ⊩o φ) → A ⊩c φ
...
\end{code}
\par Now that we have a grasp on the outer layer of the |algorithm|, we take a look at one layer down: what will go inside the cases of |algorithm|, which cases on the constructors of the given tree decomposition. First, we need to trudge through some background code that is used throughout these lemmas. Recall that |fixed| and |fixed1| are the types for bags in a tree decomposition, re-defined below. |fixed| is just a proof that for given subsets $A1$ and $A2$, there exists a subset $X$ such that $X$ is in each of $A1$ and $A2$ and X is decidable. Similarly, |fixed1| says that given a subset $A1$, there exists a subset $X$ such that $X \subset A_{1}$ and X is decidable. |fixed2fixed1| and |fixed2fixed2| show that |fixed A1 A2| implies |fixed1 A1| and |fixed1 A2|. |fixed2union| asserts that if you have |fixed A1 A2|, then you must have |fixed1(union A1  A2)|. Then, |fixed1Sub| asserts that if $X$ is a possible bag of a set $A1$ and $A1 \subset A2$ then $X$ could be a bag of |A2| as well. |decSingleton| states that a singleton set is always decidable. |IndividSinSubset| is essentially a dance around Agda's strict type system. It says that if an element |x| is in a given subset |A|, then the singleton set $|{x}|$ is a subset of |A|. These lemmas are unimportant as they are just small, obvious proofs to aid the larger ones; they are outlined so as not to confuse the reader but we do not include their code.
\par As mentioned earlier, we translate Lemma 11 into Agda with two functions, one to be used in the case that we are combining two subgames from the join node of a tree decomposition called |combineJoin|, and one to be used in the case that we are combining two games in an introduce node of a tree decomposition, called |combineIntro|. The types of these functions are below. See that |combineJoin| takes a structure $\mathds{A}$, two decidable subsets $B_{1}$ and  $B_{2}$ (where $\mathds{A}_{1}$ in the definition of the lemma is supposed to be represented by $\mathds{A}[B_{1}]$ and similar for $B_{2}$), a bag $X$ (|fixed B1 B2|), and proofs that both $B1 \subset A$ and $B2 \subset A$, and two undecided reduced games, $reduced(\mathds{A}[B_{i}] \phi X)$ for $i ∈ {1, 2}.$ |combineJoin| returns either a proof that the verifier has a winning strategy for $EMC(\mathds{A}[B_{1}], X, \phi)$, a proof that the falsifier has a winning strategy for that game, or that the game is reduced and undecided. Note that the result type is the same as that of the overarching |algorithm|, as required because |algorithm| returns the result of |combineJoin|. The proof of this function is in process, as with the rest outlined in this chapter.
\begin{code}
 combineJoin : ∀ {Σ} {oc} (B1 B2 : Subset) (A : Structure oc Σ)
                          (φ : Formula Σ)
                          → (decb1 : DecidableSub B1) (decb2 : DecidableSub B2) (X : fixed B1 B2)
...
\end{code}
|combineIntro| is quite similar to |combineJoin|. See that |combineIntro| takes a decidable subset $B$; a structure $\mathds{A}$, a set element x of A, (where $\mathds{A}_{1}$ in the definition of the lemma is supposed to be represented by $\mathds{A}[B]$ and similar for $A_{2}$ and $[X \cup {x}]$), a proof that $x \notin B$,  a bag $X$ (|fixed B|), a proof that $B \subset A$, and an undecided reduced game, $reduced(\mathds{A}[B], X, \phi )$, and a naive (i.e. brute force) algorithm of whether the structure restricted to the bag $X \cup {x}$ satisfies the formula. |combineIntro| returns either a proof that the verifier has a winning strategy for $EMC(\mathds{A}[B \cup {X}], X\cup {x}, \phi)$, a proof that the falsifier has a winning strategy for that game, or that the game is reduced and undecided. Note that the result type is again the same as that of the overarching |algorithm|, as required because |algorithm| returns the result of |combineIntro|.
\begin{code}
combineIntro : ∀ {Σ} {τ} (B : Subset) (A : Structure Closed Σ) (φ : Formula Σ)
                                        (x : IndividS (fst A) τ)
...
\end{code}
\par Now we have |combineForget|, which corresponds to Lemma 12 in the paper and is used in the case where the tree decomposition in the function |algorithm| is a forget node. |combineForget| takes a decidable subset $B$; a structure $\mathds{A}$, a $\Sigma$-formula $\phi$, a bag X (fixed B), a set element x of A, a proof that $x \notin X$, and a reduced undecided game $reduced(A[B], X \cup {x}, \phi)$, (which is supposed to represent the result of a recursive call to |algorithm| for a node below the current node of the tree decomposition). |combineForget| only returns a reduced undecided game $reduced(A[B], X, \phi)$, because Lemma 12 states that the game needs to be undecided for the lemma to hold.
\begin{code}
 combineForget : ∀ {Σ} {τ} (B : Subset) (A : Structure Closed Σ)  (φ : Formula Σ)
                           (X : fixed1 B) (x : IndividS (fst X) τ)
                           (decB : DecidableSub B) (bsubAset : Sub B (fst A))
                           (xgone : (Sub (singleton {τ = τ} (fst x))) (complement (fst X))) →
                           (recurcall :   (provesR (restriction A B decB bsubAset) φ (X)))
                         → (provesR (restriction A B decB bsubAset) φ X )
\end{code}
\par As noted in the mathematical proof, there is no need for a combine algorithm for the base case, because we simply call |naive|, the brute force algorithm of determining if a formula holds on a  structure. (For those who are familiar with the Kneis et al. paper, this is our eval.) The type of |naive| is included below: see that takes a formula $\phi$ , a structure $\mathds{A}$, a bag X, and returns whether the fasifier or the verifier has a winning strategy on  $EMC$($\mathds{A}, X, \phi)$, or if the game is undecided. Unlike the combine algorithms, there is no fancy footing here: this algorithm is exponential time, but as outlined above, it only is used on sets of constant size.
\begin{code}
naive : ∀ {Σ'} {oc} → (φ : Formula Σ') (A : Structure oc Σ') → (X : fixed1 (fst A))
                           → Either (Either (A ⊩o φ) (A ⊩o φ false)) (provesR A φ X)
\end{code}
\par After the proof of the overall algorithm is finished, we plan to formalize in Agda how the algorithm outlined meets the FPT criteria for Courcelle's Theorem. After we verify the time bound, then we will move on to verifying some applications of this theorem---some promising directions seem to be in phylogenetics and other graph algorithms, especially those having to do with graph minor theory as explained in the related work section.
\chapter{Conclusion}
\par We are well en-route to completing a verification in Agda of Courcelle's Theorem, a general graph algorithm stating that questions definable in Monadic Second Order Logic can be decided in fixed-parameter tractable time on graphs of bounded treewidth. We have encoded all of the necessary definitions for this project, namely symbols (|SigThing|), signatures (|Signature|), structures (|Structure|), restrictions of structures (|restriction|), isomorphisms between structures (|preserves|, |iso|) set elements (|Individ| and |IndividS|), sets (|Subset|), tree decompositions \linebreak (|TreeDecomp|), model checking games (|⊩c|), extended model checking games (|⊩o|), position equivalence and game equivalence (|positionEquiv| and |gameEquiv|), and finally, educed games (|isRed|, |⊩s|, |provesR|). We are in the process of constructing the naive, or brute force algorithm (|naive|), which will run only on leaves of the tree decomposition and restrictions of the input structure of bags of the tree decomposition. We have defined the types for all of the lemmas required to prove the algorithm overall (|algorithm|, |provesRtoClosed|, |openToClosed|, |combineIntro|, |combineJoin|, |combineForget|), and the proofs of these types is in progress as well. Once this is completed, we hope to prove the time bound of the algorithm, and then verify its use in the graph minor decomposition paper by Reed et al.
\par This thesis could become useful if Courcelle's Theorem becomes practicable in the field of relational databases, as the fact that it is verified could provide a sense of security to manipulating sensitive data more quickly. It could also allow Bruno Courcelle to take a quick sigh of relief, since Agda agrees that his proof is correct.
% End {{{
