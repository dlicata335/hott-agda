...
\par While we will prove our own version of the overall proof of the algorithm with our code, we outline the proof suggested by Kneis et al. here\cite{kneis}, which is never explicitly put together for this method of proving Courcelle's Theorem. However, in an effort not to reproduce too much of their paper, we simply rely upon several lemmas from the paper by Kneis et al. We reproduce the statement of the lemmas below, but for the proof, please refer to the Kneis et al. paper. Additionally, note that we refer to two algorithms not mentioned yet, \textit{combine} and \textit{combine-forget}. These are algorithms laid out in the Kneis et al. paper which we will not reproduce, but please think of them as similar to the agda functions |combineIntro|, |combineJoin|, and |combineForget| which will be described below, after the proof.
\par We show that given a $\Sigma$-structure $\mathds{A}$ that fully interprets $\Sigma$, a $\Sigma$-formula $\phi \in MSO(\Sigma)$, and a tree decomposition \textbf{T} on $\mathds{A}$ and $\phi$, we can return a game that determines whether or not $\mathds{A}$ satisfies $\phi$. The following are the lemmas from the Kneis et al. paper that we use\cite{kneis}:
