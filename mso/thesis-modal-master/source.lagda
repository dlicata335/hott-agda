...
\par We can see that a tree decomposition of a graph is an encoding of a graph into a tree: it preserves all the information of the original graph within it. It has all of the vertices of the original graph by the first condition. It keeps track of all of the edges in the graph by the second condition. To see why the third condition is necessary, we need to give an overview of what the algorithm given by Courcelle's Theorem looks like. The idea is that we check in input formula $\phi$ at each bag $X_{i}$ of a tree decomposition, and merge that information in a way which will be explained later. In light of this, to illustrate why condition three is needed, consider the two figues below. We have a graph $G$ that is a triangle, and a "tree decomposition" $T$ which does not satisfies the first and second, but not the third, condition of tree decompositions.
\begin{figure}[h]
\caption{Our graph G}
...
\par A \textbf{structure} $\mathds{A}$ is a tuple $\mathds{A} = ( A, (R^{\mathds{A}})_{R\in rel(\Sigma)}, (c^{\mathds{A}})_{c\in constants(\Sigma)})$, where $A$ is a finite set called the universe of $\mathds{A}$, and $R^{\mathds{A}}$ and $c^{\mathds{A}}$ are interpretations of the relations and constants in $\Sigma$ using the elements of $A$. More precisely, $R^{\mathds{A}} \subset A^{arity(R)},$ and $c^{\mathds{A}}$ is either an element of $A$, or $c^{\mathds{A}}$=nil.  \par Note that a constant can be interpreted as $nil$. If a structure $\mathds{A}$ interprets some $c^{\mathds{A}}$ as $nil$, we say that $c^{\mathds{A}}$ is uninterpreted. The structure leaves a blank spot at that symbol $c^{\mathds{A}}$, it does not interpret it as anything. We say that $c\in interpreted(\mathds{A})$ if and only if $c^{\mathds{A}} = a\in A$, i.e.  $c^{\mathds{A}} \neq nil.$ We often call the constants $c$ nullary constants because they have arity zero.
...
\par After defining |Subset|, we defined |IndividS|, which is simply an |Individ| together with the data that it's in a specified |Subset|. This will become useful later on.
\begin{code}
...
\end{code}
\par We also defined a datatype |OC|, with constructors open and closed. This datatype will be part of the information the Agda type for structures carry with them. A structure being open corresponds to possibly it having constants intepreted as $nil$, and a structure being closed means that all constants in $\Sigma$ are interpreted in $\mathds{A}$.
...
|StructureS| has four constructors: empty (|[]|), an expansion with an element of the underlying set (|,is|, extention with |IndividS A τ|),
...
\end{code}
\par The |Empty| constructor corresponds to the mathematical definition of the leaf of a tree decomposition. It has an empty set for its bag and an empty set for the elements below. This corresponds to the definitoin of a nice tree decomposition given above: the leaves are required to have no nodes below them, necessitating that $B = \bigcup_{\text{j at or below i}}X_{j}=\emptyset$, and and also to have empty bags, necessitating that $X_{i}=\emptyset$.
...
\end{code}
\par Let's see how the Agda definitons |preserves| corresponds to the definiton of isomorphism above, except for the condition that $h$ (|f|) is a bijection.
 We prove that this by induction over the signature $\Sigma$, which corresponds to the proof in the code: each condition in the definition above corresponds to one (or two) cases in the code.
...
\end{itemize}
\par A game $G$ is said to be determined if either one of the players has a winnings strategy, otherwise $G$ is undetermined. The game returns either some indication either player 1 or player 2 has a winning strategy, or a proof that the game is a draw.
\end{definition}
...
\par Finally, we can come to our code for closed truth. |⊩c| requires a $\Sigma$-structure $(A , SA)$ and $\Sigma$-formula $\phi$.  The input $\Sigma$-structure needs to be closed (recall the datatype OC defined way above: this means, as is required in the definition of MC games, that no constants in $\Sigma$ are interpreted as nil in $\mathds{A}$).
\begin{code}
...
\par If $\phi = \exists c \psi$, for some constant symbol $c$, then in order for $(A , SA) \Vdash c\phi$, there needs to exist some $a \in A$, i.e. some a : IndividS A $\tau$, such that $(A , (SA \space ,is \space a)) \Vdash c\psi$. (The $\Sigma  a \rightarrow$ in the code corresponds to "exists an a such that"). This, in turn, corresponds to the fact that in order for the verifier to have a winning strategy on $MC(\mathds{A},\exists c \psi)$, there needs to be at least one $a \in A$, one next move, such that the verifier has a winning strategy on the subgame $MC(\mathds{A_{a}},\psi)$. A very similar explanation works with the cases $\phi \in  {\forall R \psi, \exists R \psi}$ where R is written in the code as p.
...
\end{itemize}
\end{definition}
\par The way that we model EMC games in the code is through a datatype |⊩o|, or open truth. Similar to our definition of closed truth above, we do away with the notion of a game and players, and just outline |⊩o| so that for a structure $\mathds{A}$ and a formula $\phi$, $\mathds{A} \Vdash o \Sigma$ if and only if there is a winning strategy for the verifier on an EMC game $EMC(\mathds{A}, X, \phi)$. Before we can get to this definition, however, we have to explain some of the code leading up to it.
\begin{code}
...
$(\mathds{A}, \phi)$ where one possible next move is to $(\mathds{A} ', \phi ')$, then there will be a branch "branch $\mathds{A}$ $\phi$  $\mathds{A} '$ $\phi '$" to in the game to show that. We will see further below in our definition of game.
...
\end{code}
\par Finally, we can look at the code for open truth, our Agda encoding of an EMC game. It has four constructors: provesu (for proving universal formulas), provese (for proving existential formulas), provesbase and provesnotbase (both for atomic formulas). As noted when we explained closed truth and MC games, we take the game element out of the EMC game. We model EMC games with a datatype |⊩o| such that for a structure $\mathds{A}$ and a formula $\phi$, $\mathds{A} \Vdash o \phi$ if and only if there is a winning strategy for the verifier on an EMC game $EMC(\mathds{A}, X, \phi)$.
\begin{code}
...
\end{code}
\par IsRed takes a structure, $\mathds{A}$, a formula $\phi$, a gametree $\mathds{A} \Vdash s \phi$, and a bag X. In the case that the gametree is a leaf, which means it has no outward branches, isRed is true vacuously and so the definition for that case is Unit, or true. In the case that the gametree $\mathds{A} \Vdash s \phi$ is a node, we have a double product type; there are three requirements appended together. The first one (the code before the first $\times$) requires that for all |StructureS|'s |A'| and |Formulas| |φ'| such that there is a branch  $bj \in bs$ that connects ($\mathds{A}$, $\phi$) $\rightarrow$ ((fst $\mathds{A}$, A'), $\phi$'), there is a reduced game on ((fst $\mathds{A}$, A'), $\phi$'), i.e.   |isRed (fst A , A') φ' (x bj prfbr) fix| holds. In case it is confusing that  (x bj prfbr) is miraculously a gametree |(fst A , A') ⊩s φ'|. recall from the definition of |⊩s| that the second argument can be thought of as a function that takes a branch and a proof that that branch is in the list of branches corresponding to some gametree, and returns a gametree on the structure that the branch points to. In the code here, we are calling that second argument "x", and we see that bj is a branch, and prfbr is a proof that bj is in bs, i.e. the list of branches of |A ⊩s φ'|. So, (x bj prfbr) is a gametree |(fst A , A') ⊩s φ'|.
...
\item there is a subgame (fst A, A'') $\Vdash s \phi$' that is reduced, i.e.  isRed (fst A , A') $\phi$' gi fi (where gi = (fst A, A'') $\Vdash s \phi$' and fix = the bag X), and importantly, that there is a proof that this subgame is equivalent to (fst A , A') $\Vdash$s $\phi$', i.e.  |gameEquiv (fst A , A') (fst A , A'')| \\ |φ' gi (x bj pfbr2) (lemma1 fix))))))))|.
\end{itemize}
\end{itemize}
...
\par This work could become useful if Courcelle's Theorem becomes more useful in the field of relational databases, as the fact that it is verified could provide a sense of security to manipulating sensitive data more quickly. It could also allow Bruno Courcelle to take a quick sigh of relief, since Agda agrees that his proof is correct.
% End {{{
