...
There are also extensions of MSO, such as ExtendedMSO and LinMSO \cite{kneis, easyprobs}, that allow for linear optimizations of problems expressible in MSO, for example minimum vertex cover and dominating set. These have also been proven to be decided on graphs of bounded treewidth in fixed-parameter tractable linear time \cite{easyprobs} %\cite{kneis}.

\par The kinds of graphs that we can ask MSO questions about are graphs of bounded treewidth. Treewidth is a measure of the extent to which a graph is "tree-like". If a graph has bounded treewidth, then all of its information can be encoded in a tree in a finite fashion.
...
\par We include an example. Consider the graph below, $G$. We can see it is certianly not a tree, as it has cycles. However, we want to encode $G$ as a tree, and preserve all of the information.
...
\par In this diagram, the large circles containing letters in them depict the node of the tree, and the letters depict the elements in the bag at that node. A quick inspection can convince the reader that the three conditions of a tree decomposition hold on this structure: certainly all vertices of the original graph are present in the union of the bags of the tree decomposition. Every edge of the original graph is present in the tree decomposition because at least one bag contains each of its incident verticies. The root contains edges $(b,g)$ and $(d,g)$, the children of the root contain $(a,b)$ and $(a,d)$, and $(d,g)$ over again, and the three children of the second child contain the data for the rest of the edges. Finally, for all $i, j, k$,  nodes of T such that $i$ is on a path between $j$ and $k$, then $(X_{j} \cap X_{k}) \subset X_{i}.$ This can be seen, for example, with the path between the nodes corresponding to bags ${b,d}$ and ${c,d,f}$. The interesction of these two bags is $d$, and every node on the path between these two nodes has $d$ as an element of their bag. This property holds throughout the tree decomposition pictured.
\par Note that this tree decomposition is not necessarily optimal, and it is definitely not unique. For example, the nodes at the bottom with singleton bags ${a}$ and ${b}$ are not necessary, all of the node and edge information is already encoded in the tree decomposition without them--so an alternate tree decomposition of the same graph would not include these nodes. The tree decomposition pictured has width two, since the size of the largest bag is three.  Note also that every finite graph $G$ has finite treewidth, because a tree decomposition of a finite graph is a tree of one node, with a bag $X_{i}$ containing all vertices of G within it, so the treewidth is no greater than the number of nodes in the graph. However, as we will soon see, using this tree decomposition would render Courcelle's Theorem useless. \par Finding the treewidth of a graph is a technically  NP-hard problem, and the version of Courcelle's Theorem that I have verified assumes that it is given a tree-decomposition of a graph as an input. Boldaender proved that finding the optimal tree decomposition of an input graph is strongly fixed-parameter tractable linear, i.e. linear in the sense that Courcelle's theorem is linear, which we will explain below \cite{lintd}. However, the algorithm originally presented by Bodleander is known to be incredibly impractical; for one thing, the constants in the algorithm are prohibitive to implementation, and also the algorithm, in turn, relies upon having a non-optimal tree decomposition to start with. Thankfully, there are several good heuristics for computing treewidth, which are covered nicely in Chapter 13 in Downey and Fellows' 2013 book Fundamentals of Parametrized Complexity \cite{fellows13} as well as several survey papers by Bodleander\cite{bodtw}. These heuristic algorithms may be the most efficient method for finding a tree decomposition as an input to our algorithm, since there is really no need that tree decomposition be entirely optimal. The width being slightly larger  will just increase the time the algorithm takes. However, the algorithm verified in this paper pays no attention to this part of the process, though other proofs of the theorem have taken a different approach. We only mention this problem here to discuss the feasibility of actually using the algorithm as a way to decide MSO-expressible hard problem on graphs of bounded treewidth quickly.
