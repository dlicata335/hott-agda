...
\par The kinds of questions we can ask are those expressible in Monadic Second Order Logic, or \textbf{MSO}. MSO is an extension of first order logic which, along with all of the capabilities of first order logic, allows up to quantify over sets. However, we cannot quantify over relations that are not sets, i.e. are not unary--this is left for second order logic. For example, if we modeled edges in a graph by a binary relation $E(x,y)$ where $x$ and $y$ are vertices, we could not ask the question: $\forall E(x,y)$  is there an element $z, z \neq y$ and $x \neq z,$ such that $E(y, z)$ or $E(x, z)$?  This is because we would be quantifying over all edges, which here we depict as a binary relation.
\par Even if we do not include quantification over edges, MSO manages to express quite a few NP hard problems: k-colorability for fixed k, vertex cover, dominating set, to name a few. Below are the MSO representations of Vertex Set and Dominating Set. $R$ is a set of verticies, in each formula.
\[\text{k-Vertex Cover}(G)=\exists R \forall x \forall y (\neg Edge(x , y) \vee x ∈ R \vee y ∈ R)
\]
\[
...
\par Finally, we come to the condition on linear-ness:  the algorithm we get for deciding an MSO question on a graph of bounded treewidth is fixed-parameter tractable linear, or FPT linear. Parametrized complexity, the field that gives us the notion of fixed parameter tractable algorithms, was developed by Robert Downey and Michael Fellows in the 1990s. Main idea in paramtertized complexity is thinking of complexity in multiple dimensions rather than just one. Downey and Fellows explain their idea as a "deal with the devil" to compensate for the inevitable combinatorial explosion in most interesting algorithms: they limit the explosion to one attribute, the parameter, of the input. [cite downey fellows book]. In our case, the inevitable exponential blowup is checking every element of an underlying set of a structure at each universal or existential quantifier is limited to the treewidth of the graph and the encoding of the MSO sentence. An FPT linear algorithm is linear with respect to the size of the input, in our case, the graph, but with an exponential constant proportional to other parameters, in our case, the encoding of the MSO question, $\phi,$ and the treewidth $\omega.$ Unfortunately, this exponential constant is very large. In fact, unless P=NP, the constant cannot be upper bounded by an iterated exponential of height bounded by $\phi$ and $\omega$.
...
\par The problem with the automata approach is that in practice, many of the states of the tree automaton generated are never used (cite). There are a few different ways to circumvent this issue; they all involve some version of creating what you need as you go. Courcelle and Durand use functions instead of tables to model automata in an effort to make the algorithm faster and dub the method using fly-automata [automata for the verification]. The name is due to the fact that the  state-transition functions are created "on-the-fly". Alternatively, this can be done by scrapping the idea of automata all together. Ganzow and Kaiser bypass the use of automata at all and directly manipulates the input formulas [new algo].  This paper uses a game theoretic method, which takes the details of the actual input graph given into account, so that the algorithm only stores and uses the information it needs.
\par This is also the approach of the algorithm we use to verify Courcelle's Theorem, the implementation in \textbf{Courcelle's Theorem: A Game Theoretic Approach by Kneiss and other people} takes. The algorithm that this paper is based off is a fundamentally new approach, developed by Kneis, Langer, and Rossmanish in Courcelle's Theorem-- A Game-Theoretic Approach. This paper uses tools from game theory to build a relatively simple dynamic programming algorithm to decide the formula on the structure. As alluded to earlier, this implementation actually has some experimental evidence showing that the algorithm runs in a usable amount of time (from a few seconds to under 24 hours) for graphs up to treewidth 12 for minimum vertex cover, dominating set, and 3-colorability.[cite] An explanation of the algorithm is in the last section of this paper, however, we give a quick intuitive idea: Essentially, in order to prevent exponential blow up for the algorithm at every quantifier (as occurs on the automata implementation of Courcelle’s Theorem), we never want to check a quantifier over the entire set of nodes and edges in a graph. Instead of looking at the whole graph at once, the algorithm in Kneiss et. al. looks at in pieces: for a structure A, the pieces are the restrictions $A[X_{i}]$ associated with each node of the tree decomposition. (Recall that for a node i of a tree decomposition, the set $X_{i}$ is the bag, or set of nodes of the original graph, associated with the node of the tree decomposition, and $A[X_{i}]$ is the restriction of A to that subset.) The algorithm that we use to prove Courcelle’s theorem traverses the given tree decomposition bottom up, checking to see if the formula holds on  $\mathds{A}[X_{i}]$ for each node, starting at the empty leaves, and adding a little bit of the set at a time.
\par Some application areas of Courcelle's Theorem include graph theoretic algorithms, for example a paper on constructing graph minor decompositions by Reed et. al. uses Courcelle's Theorem in the proof [cite reed paper]; phylogenetics [cite slideshow] and databases, since Courcelle's Theorem promises a faster implementation for SQL queries.[cite]
...
\par It has been proven that the verifier has a winning strategy on a model-checking game $MC(\mathds{A}, \phi)$ if and only if $\mathds{A} \vdash \phi$ [cite].
