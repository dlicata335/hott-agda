...
\chapter{Helpful Background and History}
...
\chapter{Outline of Kneis et. al. Algorithm}
...
\section{Relating Structures: Isomorphisms, Compatibility, Union}
...
\par You might ask, why are there not instances of where one structure extends with a relation and another with a constant or a nil? This is because the structures are mandated to have the same signature.
\begin{code}
...
\end{code}
\par Based on these different types of formulae, we defined a notion of a branch, of which there are universal branches and existential branches. Think of a branch as an edge between two nodes in a game: it keeps track of which formula and structure the game is are traveling from and towards. (For as the algorithm goes through a game, the formula shrinks and the structure grows). So if there is a node (i.e. a position in the definition above) %is this right??
...
\chapter{Outline of Algorithm and Work in Progress}
\section{Mathematical Proof of Algorithm}
