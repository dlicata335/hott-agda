...
--sublemma
  ...
  mutual
    ...
    positionEquiv-fn :  ∀ {Σ oc1 oc2} {A1 : Structure oc1 Σ} {A2 : Structure oc2 Σ} {X : Subset} {x1 : Sub X (fst A1)} {x2 : Sub X (fst A2)} →  positionEquiv A1 A2 X x1 x2  →
                                                                      Sub (union (constants A1) (X)) (union (constants A2) (X))
    positionEquiv-fn poseq = subLUB {!subINL!} {!!}
    ...
    positionEquiv-fnS {A1 = A1} {A2 = A2} poseq {x :: τ} ind = (positionEquiv-fnS {A1 = A1} {A2} poseq {τ} (fst ind)) , promoteIndividS (positionEquiv-fn {A1 = A1} {A2} poseq) (snd ind)
    ...
    data EquivInd {Σ oc1 oc2 τ} {A1 : Structure oc1 Σ} {A2 : Structure oc2 Σ} {X : Subset} {XinA1 : Sub X (fst A1)} {XinA2 : Sub X (fst A2)}
       ...
       eqindC : ( ca1 : constants A1 τ (fst a1)) → constants A2 τ (fst a2) → _==_ {Individ τ} (fst (promoteIndividS (positionEquiv-fn {A1 = A1} {A2 = A2} {X = X} {XinA1} {XinA2} poseq) {!!} )) (fst  a2) → EquivInd poseq a1 a2
