...
module HomotopyCanonHSet8 where
  ...
  data Tm where
    ...
    lam=  : ∀ {Γ A B} {A* : Ty Γ A} {B* : Ty (Γ , A) B} 
           ...
           → Tm Γ (id _ f g)
    subst1 : ∀ {Γ A B} → {A* : Ty Γ A} {B* : Ty (Γ , A) B} → Tm (Γ , A) B* → (M : Tm Γ A*) → Tm Γ (subst1 B* M)
  ...
  interp (unlam M) (θ , N) = interp M θ N
  interp (refl M) θ = id
  ...
  interp (subst1 M N) θ = interp M (θ , interp N θ)
...
--  R ((Γ* , B*) , w _ A*) (ex A1* C*) ((rθ , rb) , ra) M = {!!}
  ...
  fund (Γ* , A0*) ._ rθ (unlam {A* = A*} {B* = B*} M) = {!fund Γ* (Π A* B*) (fst rθ) M _ (R-deq Γ* A0* A* (fst rθ) (snd rθ)) !}
