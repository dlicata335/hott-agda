...
module Coprod where
  ...
  module Coprod (A : Type) (B : Type) where
    ...
    coprodt = Paths (Either A B)         ≃〈 contract-Paths-eqv 〉 
              Either A B                 ≃〈 coe-equiv (ap2 Either (ua (!equiv contract-Paths-eqv)) (ua (!equiv contract-Paths-eqv)))  〉 
    ...
    pres-id-l x = fst coprodt ((Inl x , Inl x) , id)                                                                         =〈 id 〉
                  fst rearrange (coe (ap2 Either (ua (!equiv contract-Paths-eqv)) (ua (!equiv contract-Paths-eqv))) (Inl x)) =〈 ap (fst rearrange) (coe-Either-l (ua (!equiv contract-Paths-eqv)) (ua (!equiv contract-Paths-eqv))) 〉
                  fst rearrange (Inl (coe (ua (!equiv contract-Paths-eqv)) x))                                               =〈 ap (λ u → fst rearrange (Inl (u x))) (type=β (!equiv contract-Paths-eqv)) 〉
                  fst rearrange (Inl (fst (!equiv contract-Paths-eqv) x))                                                    =〈 id 〉
                  ((Inl x , Inl x) , id) ∎

    pres-id-r : (x : B) → (fst coprodt ((Inr x , Inr x) , id)) == ((Inr x , Inr x) , id)
    pres-id-r x = fst coprodt ((Inr x , Inr x) , id) =〈 id 〉
                  fst rearrange (coe (ap2 Either (ua (!equiv contract-Paths-eqv)) (ua (!equiv contract-Paths-eqv))) (Inr x)) =〈 ap (fst rearrange) (coe-Either-r (ua (!equiv contract-Paths-eqv)) (ua (!equiv contract-Paths-eqv))) 〉
                  fst rearrange (Inr (coe (ua (!equiv contract-Paths-eqv)) x))                                               =〈 ap (λ u → fst rearrange (Inr (u x))) (type=β (!equiv contract-Paths-eqv)) 〉
                  fst rearrange (Inr (fst (!equiv contract-Paths-eqv) x))                                                    =〈 id 〉
                  ((Inr x , Inr x) , id) ∎
