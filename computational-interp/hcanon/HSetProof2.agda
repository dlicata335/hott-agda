...
-}
  ...
  fund-transport-inv-2 : ∀ {Γ C θ1 θ2 δ N} (Γ* : Ctx Γ) (C* : Ty Γ* C) 
          ...
          (rδ : QC Γ* rθ1 rθ2 δ) 
          (rN : R Γ* C* rθ2 N) 
        → Q Γ* C* rθ2 (fund-transport Γ* C* rθ1 rθ2 rδ (fund-transport Γ* C* rθ2 rθ1 (fund-syms Γ* rθ1 rθ2 rδ) rN)) rN (ap≃ (transport-inv-2 C δ))
  ...
  fund-transport {δ = δ} {N = f} Γ'* (Π {A = A} {B = B}A* B*) rθ1 rθ2 rδ rf = 
          (λ x rx → transportR (Γ'* , A*) B* (rθ2 , rx) (! (ap≃ (transport-Π A (λ γ a → B (γ , a)) δ f))) 
                                 (fund-transport (Γ'* , A*) B*
                                  ...
                                  (fst rf _ (fund-transport Γ'* A* rθ2 rθ1 (fund-syms Γ'* _ _ rδ) rx)))) , 
...
-}
  ...
  fund-transport-id Γ* (Π A* B*) rθ rN = λ x rx → {!fund-transport-id (Γ* , A*) B* (rθ , rx) (rN x rx)!}
