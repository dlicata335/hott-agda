...
module HomotopyCanonHSet6 where
  ...
  subst1 {A* = A*} B* M = subst (· , A*) (<> , deq M) B*

  ex : ∀ {Γ A B C} → (A* : Ty Γ A) (B* : Ty Γ B) 
     → Ty ((Γ , A) , B o fst) C → Ty ((Γ , B) , A o fst) (\ θ → C ((fst (fst θ) , snd θ) , snd (fst θ)))
  ex A* B* C* = {!subst ((· , A*) , w A* B*) ? C*!} 

  unlam{Γ}{A}{B}{A*}{B*} f = deq (app {A* = (w A* A*)} {B* = (ex A* A* (w (w A* A*) B*))} (deq (w {A* = A*} f)) v) 
