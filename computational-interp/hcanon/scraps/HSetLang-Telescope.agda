...
module computational-interp.hcanon.HSetLang-Telescope where
  ...
  data TelescopeTy : ∀ {Γ Δ} (Γ* : Ctx Γ) → (Δ* : Ctx Δ) → MetaType -- proof that Δ* is an extension of Γ*
--  data PathSubst : ∀ {Γ Γ'} {Γ* : Ctx Γ} {Γ'* : Ctx Γ'} (θ1 θ2 : Subst Γ* Γ'*) → MetaType
  ...
  data TelescopeTy where
    ...
    _,_ : ∀ {Γ Δ A} {Γ* : Ctx Γ} {Δ* : Ctx Δ} → (Δ** : TelescopeTy Γ* Δ*) → (A* : Ty Δ* A) → TelescopeTy Γ* (Δ* , A*)

  substt : ∀ {Γ Γ' A} {Γ* : Ctx Γ} {A* : Ty Γ* A} {Γ'* : Ctx Γ'} (M : Tm Γ* A*) (Δ : TelescopeTy (Γ* , A*) Γ'*) → Σ \ Γ'M → Σ \ (Γ'M* : Ctx Γ'M)  → TelescopeTy Γ* Γ'M*
  substt {Γ* = Γ*} M · = _ , Γ* , ·
  substt M (Δ , A*) with substt M Δ
  ... | (Γ'M , Γ'M* , Δ*) = _ , (Γ'M* , {!!}) , {!!}
  ...
  data Ty where
    ...
    subst : ∀ {Γ Γ' A B} {Γ* : Ctx Γ} {A* : Ty Γ* A} {Γ'* : Ctx Γ'} (Δ* : TelescopeTy (Γ* , A*) Γ'*)
               (B* : Ty Γ'* B)
               (M : Tm Γ* A*) → Ty Γ* {!fst (substt M Δ*)!}
  ...
  data Tm where
    ...
    tr   : ∀ {Γ A Γ' C} {Γ* : Ctx Γ} {A* : Ty Γ* A} {Γ'* : Ctx Γ'} → (Δ* : TelescopeTy (Γ* , A*) Γ'*)
           (C* : Ty Γ'* C) {M* : Tm Γ* A*} {N* : Tm Γ* A*} (α : Tm Γ* (id A* M* N*) )
         → Tm Γ* (subst Δ* C* M*) → Tm Γ* (subst Δ* C* N*)
