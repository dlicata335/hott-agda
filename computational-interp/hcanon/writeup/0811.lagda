...
--  transportR Γ* _ rθ p q = {!!}
  ...
  transportRQ : ∀ {Γ A θ M1 M2} {α : M1 == M2} (Γ* : Ctx Γ) (A* : Ty Γ* A) (rθ : RC Γ* θ) (rM1 : R Γ* A* rθ M1) (rM2 : R Γ* A* rθ M2) 
              → Q Γ* A* rθ (transportR Γ* A* rθ α rM1) rM2 id
  transportRQ = {!!}
...
--  fund-tr Γ* (subst Γ'* θ'* A₂) rθ rM1 rM2 rα M = {!!}
  ...
  fund-ap {α = α} Γ* {A* = A*} v rθ rM1 rM2 rα = 
          {! rα !}where 
          -- transportQ Γ* A* rθ _ rM2 (coh α)
          --                           (fund-trans {α = ap≃ (transport-constant α)} {β = α} Γ* A* rθ _ _ _ {! (fund-transport-constant Γ* A* rθ rM1 rM2 α) !}
          --                            rα)
