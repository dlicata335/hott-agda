...
module misc.Retraction3f where
 ...
 module Monadic (B : Type) (b0 : B)
  ...
  Good b _ = Unit
  Good (τ1 ⇒ τ2) r = 
    (( x : << τ1 >>) → (r (merge τ1 (split τ1 x)) == (r x)) × Good τ2 (snd (r x)))

  Good-normal : (τ : Tp) (x : << τ >>) → Good τ x → merge τ (split τ x) == x
  Good-normal b x gx = id
  Good-normal (τ1 ⇒ τ2) x gx = λ≃ (λ x₁ → ap2 _,_ (ap fst (fst (gx x₁))) (ap snd (fst (gx x₁)) ∘ {!BROKEN?!}))
