...
module misc.Retraction3e where
 ...
 module Monadic (B : Type) (b0 : B)
  ...
  Good : (τ : Tp) → << τ >> → Type
  Good b _ = Unit
  Good (τ1 ⇒ τ2) r = (( x : << τ1 >>) → Good τ1 x → (merge⇒ τ1 τ2 (split⇒ τ1 τ2 r) x == r x) × Good τ2 (snd (r x)))

  Good-stable : (τ : Tp) (x : << τ >>) → Good τ x → merge τ (split τ x) == x
  Good-stable b x _ = id
  Good-stable (τ1 ⇒ τ2) f gf = λ≃ (λ x → {!!}) 
...
-}
  ...
  thm : ∀ {Γ} {τ} (e : Γ ⊢ τ) (θ : _) → RStable (retc Γ) θ → << e >>e θ == Retraction.g (ret⊢ Γ τ) ([[ e ]]e , << e >>split) θ
  thm c θ sθ = id
  thm {τ = τ} (v i0) θ sθ = ap (λ x → c0 , x) (! (ap snd sθ))
  thm (v (iS x)) θ sθ = thm (v x) (fst θ) (ap fst sθ)
