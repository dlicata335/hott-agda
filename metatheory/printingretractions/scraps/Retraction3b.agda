...
module misc.Retraction3b where
 ...
 module Monadic (B : Type) (b0 : B)
  ...
  ImMerge : (τ : Tp) → << τ >> → Type
  ImMerge τ x = x == merge' τ (split τ x)

  HImMerge : (τ : Tp) → << τ >> → Set
  HImMerge b p = Unit
  HImMerge (τ1 ⇒ τ2) f = ImMerge (τ1 ⇒ τ2) f × ((x : << τ1 >>) → HImMerge τ1 x → HImMerge τ2 (snd (f x)))

  use-HImMerge : (τ : Tp) (x : << τ >>) → HImMerge τ x → ImMerge τ x
  use-HImMerge b x x₁ = id
  use-HImMerge (τ1 ⇒ τ2) f imf = fst imf

  HImMergeC : (Γ : Ctx) → << Γ >>c → Set
  HImMergeC [] θ = Unit
  HImMergeC (τ :: Γ) θ = HImMergeC Γ (fst θ) × HImMerge τ (snd θ)

  use-HImMergeC : (Γ : Ctx) (x : << Γ >>c) → HImMergeC Γ x → x == mergec' Γ (splitc Γ x)
  use-HImMergeC [] θ hθ = id
  use-HImMergeC (τ :: Γ) θ hθ = ap2 _,_ (use-HImMergeC Γ (fst θ) (fst hθ)) (use-HImMerge τ (snd θ) (snd hθ))

  mutual 
    immerge : ∀ {Γ}{τ} (e : Γ ⊢ τ) → (θ : << Γ >>c) 
            → HImMergeC Γ θ → HImMerge τ (snd (<< e >>e θ))
    immerge c θ iθ = <>
    immerge {τ = τ} (v i0) θ iθ = snd iθ
    immerge (v (iS x)) θ iθ = immerge (v x) (fst θ) (fst iθ)
    immerge (lam{τ1}{τ2} e) θ iθ = (λ≃ (λ x → ap2 _,_ {!!} {!(use-HImMerge τ2 _ (immerge e (θ , merge' τ1 (split τ1 x)) (iθ , ?)))!}) , (λ x hx → immerge e (θ , x) (iθ , hx)))
    immerge (app e e₁) θ iθ = {!!}
{-
  allindep : ∀ {Γ}{τ} (e : Γ ⊢ τ) 
