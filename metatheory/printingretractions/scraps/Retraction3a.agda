...
module misc.Retraction3a where
 ...
 module Monadic (B : Type) (b0 : B)
  ...
  ImMerge : (τ : Tp) → << τ >> → Type
  ImMerge τ x = Σ \ pc → x == merge' τ pc

  -- potential part of the output is independent of the cost part of the input
  ImMergeMap : (τ1 τ2 : Tp) (r : << τ1 >> → << τ2 >>) →  Set
  ImMergeMap τ1 τ2 r = (x : << τ1 >>) → ImMerge τ1 x → ImMerge τ2 (r x)

  HImMerge : (τ : Tp) → << τ >> → Set
  HImMerge b p = Unit
  HImMerge (τ1 ⇒ τ2) f = ImMergeMap τ1 τ2 (snd o f) ×
                         ((x : << τ1 >>) → HImMerge τ1 x → HImMerge τ2 (snd (f x)))

  ImMergeC : (Γ : Ctx) → << Γ >>c → Set
  ImMergeC [] θ = Unit
  ImMergeC (τ :: Γ) θ = ImMergeC Γ (fst θ) × ImMerge τ (snd θ)

  ImMergeMapC : (Γ : Ctx) (τ : Tp) (r : << Γ >>c → << τ >>) →  Set
  ImMergeMapC Γ τ r = (θ : << Γ >>c) → ImMergeC Γ θ → ImMerge τ (r θ)

  mutual 
    immerge : ∀ {Γ}{τ} (e : Γ ⊢ τ) → ImMergeMapC Γ τ (snd o << e >>e) 
    immerge c θ iθ = (b0 , <>) , id
    immerge (v i0) θ iθ = snd iθ
    immerge (v (iS x)) θ iθ = immerge (v x) (fst θ) (fst iθ)
    immerge (lam{τ1}{τ2} e) θ iθ = ((λ x → fst (fst (immerge e (θ , merge' τ1 (x , default τ1)) (iθ , _ , id)))) , 
                                   (λ pot cst → fst (<< e >>e (θ , merge' τ1 (pot , cst))) , snd (fst (immerge e (θ , merge' τ1 (pot , cst)) (iθ , _ , id))))) , 
                                  λ≃ (λ x → ap2 _,_ {!!} {!!})
    immerge (app e e₁) θ iθ = {!!}

    himmerge : ∀ {Γ}{τ} (e : Γ ⊢ τ)  (θ : << Γ >>c) → ImMergeC Γ θ → ImMerge τ (snd (<< e >>e θ))
    himmerge e θ iθ = {!!}
{-
  allindep : ∀ {Γ}{τ} (e : Γ ⊢ τ) 
