...
module adjointlogic.Idempotent-Subcalculus2 where
  ...
  data _[_]⊢'_ : Tp c → (c ≥ c) → Tp c → Set where
    ...
    hypq' : Q [ 1m ]⊢' Q
    copy' : ∀ {A} → A [ r ]⊢' A
    ♯R' : ∀ { A B } → A [ r ]⊢' B → A [ 1m ]⊢' ♯ B -- dual restriction; kinda weird
    ♯L' : ∀ { A B } → A [ r ]⊢' B → ♯ A [ r ]⊢' B
    ♭L' : ∀ {A B} → A [ r ]⊢' B → ♭ A [ 1m ]⊢' B
    ♭R' : ∀ {A B} → A [ r ]⊢' B → A [ r ]⊢' ♭ B
  ...
  copy'' : ∀ A → A [ r ]⊢' A -- not admissible if it's not a rule
  copy'' P = {!!}
  copy'' Q = {!!}
  copy'' (F {q = q} α A) with 0-cell-case {q} 
  copy'' (F α A) | id with 1-cell-case α
  copy'' (F .1m A) | id | Inl id = {!!}
  copy'' (F .r A) | id | Inr id = {!♭L'!} -- can't use ♭L'
  copy'' (U {p = p} α A) with 0-cell-case {p} 
  copy'' (U α A) | id with 1-cell-case α 
  copy'' (U .1m A) | id | Inl id = {!!}
  copy'' (U .r A) | id | Inr id = {!!}
  copy'' (A ⊕ A₁) = {!!}
  ...
  mutual 
    ...
    cut' (♭R' D) (♭L' E) = cut' D E
    cut' (♯R' D) (♯L' E) = cut' D E
    cut' (Inl' D) (Case' E1 E2) = {!!}
    cut' (Inr' D) (Case' E1 E2) = {!!}
    -- copy
    cut' copy' hypp' = {!!} -- irreducible
    cut' copy' hypq' = {!!} -- irreducible
    cut' copy' copy' = copy'
    cut' copy' (♯R' E) = {!♯R'!} -- stuck
    cut' copy' (♯L' E) = ♯L' E
    cut' copy' (♭L' E) = adj'r (♯R' E) -- FIXME definitely not smaller
    cut' copy' (♭R' E) = ♭R' E
    cut' copy' (Inl' E) = Inl' (cut' copy' E)
    cut' copy' (Inr' E) = Inr' (cut' copy' E)
    cut' copy' (Case' E E₁) = cut' (Case' (♯R' (cut' copy' E)) (♯R' (cut' copy' E₁))) (♯L' copy') -- FIXME
    cut' D copy' = {!!}
    -- right commutative
    cut' hypp' hypp' = hypp'
    cut' hypq' hypq' = hypq'
    cut' D (♭R' E) = transport (λ x → _ [ x ]⊢' _) (! (∘r _)) (♭R' (transport (λ x → _ [ x ]⊢' _) (∘r _) (cut' D E)))
    cut' {β = β} D (♯R' E) with 1-cell-case β
    cut' D (♯R' E) | Inl id = ♯R' (cut' D E)
    cut' D (♯R' E) | Inr id = cut' copy' (♯R' (cut' D E))  -- FIXME
    cut' D (Inl' E) = Inl' (cut' D E)
    cut' D (Inr' E) = Inr' (cut' D E)
    -- left commutative
    cut' (♯L' D) E = transport (λ x → _ [ x ]⊢' _) (! (r∘ _)) (♯L' (transport (λ x → _ [ x ]⊢' _) (r∘ _) (cut' D E)))
    cut' {α = α} (♭L' D) E with 1-cell-case α
    cut' (♭L' D) E | Inl id = ♭L' (cut' D E)
    cut' (♭L' D) E | Inr id = cut' copy' (♭L' (cut' D E))  -- FIXME
    cut' {α = α} (Case' D1 D2) E with 1-cell-case α 
    cut' (Case' D1 D2) E | Inl id = Case' (cut' D1 E) (cut' D2 E)
    cut' (Case' D1 D2) E | Inr id = cut' (Case' (♯R' (cut' D1 E)) (♯R' (cut' D2 E))) (♯L' copy') -- FIXME

    adj' : ∀ {A C} → A [ 1m ]⊢' ♯ C → ♭ A [ 1m ]⊢' C
    adj' D = ♭L' (cut' D (♯L' copy'))
  
    adj'r : ∀ {A C} → A [ 1m ]⊢' ♯ C → ♭ A [ r ]⊢' C
    adj'r D = cut' copy' (♭L' (cut' D (♯L' copy')))

    ♭L'r : ∀ {A B} → A [ r ]⊢' B → ♭ A [ r ]⊢' B
    ♭L'r {A}{B} D = cut' (♭L' (cut' copy' (♯R' D))) (♯L' copy') 

    ⊕Lr : ∀ {A B C} → A [ r ]⊢' C → B [ r ]⊢' C → (A ⊕ B) [ r ]⊢' C
    ⊕Lr D1 D2 = cut' (Case' (♯R' D1) (♯R' D2)) (♯L' copy')
