...
module functorlogic.NDBI where
  ...
  subst var (▸ D) = D
  subst {Γ = Γ} {Γ'} (FE {α = α} s D E) θ with ⇉s s θ
  subst (FE s D E) θ | (Δ1 , (Δ2 [ .β ] , (sΔ , (θ1 , (θ2 [ β ]))))) = FE sΔ (subst D θ2) (subst E (subst, split, θ1 (▸ var [ _ ]))) 
  ...
  reduce (FE s (FI E) E₁) = subst E₁ (subst, s (ids _) {!need fusion!}) -- subst E₁ (subst, s (ids _) (▸ E [ _ ]))
  ...
  F∘2 : ∀ {p q r : Mode} {A : Tp r} {α : r ≥ q} {β : q ≥ p}
      → · ⊢ (F (α ∘1 β) A ⊸ (F β (F α A)) )
  F∘2 {A = A}{α = α}{β} = Lam (FE {Γ1 = ·} {Γ2 = ▸ (F (α ∘1 β) A)} {β = 1m} {!!} var {!FI!})
