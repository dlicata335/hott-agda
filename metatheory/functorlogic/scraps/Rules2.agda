...
module functorlogic.Rules2 where
  ...
  data _[_]⊢_[_] : {p q r : Mode} → Tp p → r ≥ p → Tp q -> r ≥ q → Set where
    ...
    Case : ∀ {p q} {C : Tp p} {γ : q ≥ p} {A B : Tp q} → A [ 1m ]⊢ C [ γ ] → B [ 1m ]⊢ C [ γ ] → (A ⊕ B) [ 1m ]⊢ C [ γ ]
    CutNatCut : ∀ {p q r s} {α : s ≥ p} {β : s ≥ q} {γ γ' : s ≥ r}
                {A : Tp p} {B : Tp q} {C : Tp r}
              → A [ α ]⊢ C [ γ ]
              → (e : γ ⇒ γ')
              → C [ γ' ]⊢ B [ β ]
              → A [ α ]⊢ B [ β ]
  ...
  func (hypp x) = hypp (1⇒ ∘1cong x)
  func (hypq x) = hypq (1⇒ ∘1cong x)
  func (FL D) = {!!}
  func (FR D) = {!!}
  func (Inl D) = {!CutNatCut !}
  func (Inr D) = {!!}
  func (Case D D₁) = {!!}
  func (CutNatCut D e D₁) = {!!}
  ...
  compose (A ⊕ B) e = {!!} -- Case (Inl 1⇒ (compose A 1⇒)) (Inr 1⇒ (compose B 1⇒))
  ...
  compose' {β1 = β1}{β2 = β2}{α1} {A = A} {B = B1 ⊕ B2} e (Inl D) = {! CutNatCut (Inl D) ? ? !}
  compose' e (Inr D) = {!!}
  compose' e (Case D D₁) = {!!}
  compose' e1 (CutNatCut D e2 E) = {!!}
  ...
  cut : ∀ {p q r s} {α : s ≥ p} {β : s ≥ q} {γ : s ≥ r} {A : Tp p} {B : Tp q} {C : Tp r}
      ...
      → A [ α ]⊢ C [ γ ]
  -- weird thing
  cut D (CutNatCut D2 e E) = CutNatCut (cut D D2) e E
  cut (CutNatCut D e E1) E2 = CutNatCut D e (cut E1 E2)
  -- -- atom
