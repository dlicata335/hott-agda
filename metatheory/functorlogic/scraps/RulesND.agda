...
module functorlogic.RulesND where
  ...
  mutual
    ...
    data _[_]⇑_[_] : {p q r : Mode} → Tp p → r ≥ p → Tp q -> r ≥ q → Set where
      Func : ∀ {p q r s t t'} {α : s ≥ p} {β : s ≥ q} {δ δ' : s ≥ r} {γ : r ≥ t} {γ' : r ≥ t'} 
                {A : Tp p} {A' : Tp t} {B' : Tp t'} {B : Tp q}
              → A [ α ]⇓ A' [ δ ∘1 γ ]
              → (e : δ ⇒ δ') 
              → A' [ γ ]⇑ B' [ γ' ]
              → B' [ δ' ∘1 γ' ]⇑ B [ β ]
              → A [ α ]⇑ B [ β ]
      ⇓⇑ : ∀ {p q s} {α α' : q ≥ p} {γ : q ≥ s} {A : Tp p} {C : Tp s}
         → C [ γ ]⇓ A [ α ]
         → (e : α ⇒ α')
         → C [ γ ]⇑ A [ α' ]
  ...
  composer : ∀ {p q s} {α α' : q ≥ p} {γ : q ≥ s} {A : Tp p} {C : Tp s}
           → C [ γ ]⇑ A [ α ]
           → (e : α ⇒ α')
           → C [ γ ]⇑ A [ α' ]
  composer (Func D1 e D2 D3) e₁ = {!!}
  composer (⇓⇑ E e) e₁ = ⇓⇑ E (e ·2 e₁)
  composer (FI D) e = FI (composer D (e ∘1cong 1⇒))

  hsubsta : ∀ {p q r s : Mode} {A : Tp p} {α : r ≥ p} {B : Tp q} {β : r ≥ q} {C : Tp s} {γ : r ≥ s}
          → A [ α ]⇑ B [ β ]
          → B [ β ]⇓ C [ γ ]
          → A [ α ]⇑ C [ γ ]
  hsubsta D V = D
  hsubsta D (FE E) with hsubsta D E
  hsubsta D (FE E) | ⇓⇑ E' e = ⇓⇑ (FE E') (e ∘1cong 1⇒)
  hsubsta D (FE E) | FI E' = E'
  hsubsta D (FE E) | Func D1 e D2 D3 = {!!}

  hsubst : ∀ {p q r s : Mode} {A : Tp p} {α : r ≥ p} {B : Tp q} {β : r ≥ q} {C : Tp s} {γ : r ≥ s}
         → A [ α ]⇑ B [ β ]
         → B [ β ]⇑ C [ γ ]
         → A [ α ]⇑ C [ γ ]
  hsubst D (Func D1 e D2 D3) = {!!}
  hsubst D (⇓⇑ E e) = composer (hsubsta D E) e
