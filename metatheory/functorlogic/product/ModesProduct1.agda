...
-- make products a datatype, because pattern matching works better that way

open import functorlogic.Lib 

module functorlogic.ModesProduct1 where
  ...
  postulate
    _≤_ : Mode -> Mode -> Set 
    1m : {m : Mode} → m ≤ m
    _·1_ : {x y z : Mode} → x ≤ y → y ≤ z → x ≤ z -- diagrammatic order
  ...
  postulate
    fstm : ∀ {p q} → (p ×m q) ≤ p
    sndm : ∀ {p q} → (p ×m q) ≤ q
    _,m_ : ∀ {p q r} → p ≤ q → p ≤ r → p ≤ (q ×m r)
    <>m  : ∀ {p} → p ≤ ⊤m

    fstβ : ∀ {p q r} → {α : p ≤ q} → {β : p ≤ r} → ((α ,m β) ·1 fstm) == α
    sndβ : ∀ {p q r} → {α : p ≤ q} → {β : p ≤ r} → ((α ,m β) ·1 sndm) == β
    ×mη  : ∀ {p q r} → {α : p ≤ (q ×m r)} → α == ((α ·1 fstm) ,m (α ·1 sndm))
    ⊤mη  : ∀ {p} → {α : p ≤ ⊤m} → α == <>m
