...
module functorlogic.NDHypOnly-NonStructural-Product3 where
  ...
  postulate
    ...
    [<>] : ∀ {p} {Γ : Ctx p} → Γ [ <>m ] == ·
    [,] : ∀ {p q r s} {α : p ≤ q} {β : p ≤ r} {γ : (q ×m r) ≤ s}{Γ : Ctx p} → Γ [ (α ,m β) ·1 γ ] == (Γ [ α ] , Γ [ β ]) [ γ ]
  ...
  ×mηc : ∀ {p q} {Γ : Ctx (p ×m q)} → Γ == (Γ [ fstm ] , Γ [ sndm ])
  ×mηc {Γ = Γ} = [,] {α = fstm} {β = sndm} {γ = 1m} ∘ ap {M = 1m} {N = fstm ,m sndm} (_[_] Γ) (×mη {α = 1m}) ∘ ! [1]
...
-}
  ...
  ids · = subst· idc
  ids (▸ A) = subst▸ (var idc)
  ids (Γ , Δ) = subst, {Γ = (Γ , Δ)} (ids Γ) (ids Δ)
  ...
  subst, θ1 θ2 ·sr (_,c_ {Δ1 = Δ1}{Δ2 = Δ2} ρ1 ρ2) = 
        subst, (θ1 ·sr ρ1) (θ2 ·sr ρ2)
