...
-- is equality composition admissible for substs?

open import functorlogic.Lib
open import functorlogic.ModesProduct1

module functorlogic.NDHypOnly-NonStructural-Product8 where
...
-}
  ...
  test : ∀ {p q} {Γ : _} {A : Tp (p ×m q)} → Γ ⊢s ▸ A
  test = subst▸ {!!}

  test2 : ∀ {p q} {Γ : _} {A : Tp (p ×m q)} → Γ ⊢s (▸ A [ fstm ] , ▸ A [ sndm ])
  test2 = subst, (subst[] idc {!!}) (subst[] idc {!!})
